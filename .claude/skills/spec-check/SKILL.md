...
Deslop-specific examples (from the actual spec): `[PARSE-CSHARP-NORMALIZE]`, `[RANK-SCORE]`, `[LIVE-WATCHER]`, `[LSP-DIAGNOSTICS]`, `[MCP-QUERY]`.
...
Code files should contain comments referencing the spec ID. The search must catch **all** comment styles across languages used in this repo (Rust, TypeScript, JavaScript, shell, TOML, YAML):

**C-style `//` comments** (Rust, TypeScript, JavaScript):
- `// Implements [AUTH-TOKEN-VERIFY]`
- `// [AUTH-TOKEN-VERIFY]`
- `// Tests [AUTH-TOKEN-VERIFY]` (also counts as a code reference)
- `/// Implements [AUTH-TOKEN-VERIFY]` (Rust doc comments)

**Hash `#` comments** (Shell/Bash, YAML, TOML, Makefile):
- `# Implements [AUTH-TOKEN-VERIFY]`
- `# [AUTH-TOKEN-VERIFY]`
- `# Tests [AUTH-TOKEN-VERIFY]`

**HTML/XML comments** (HTML templates in `site/`, SVG):
- `<!-- Implements [AUTH-TOKEN-VERIFY] -->`
- `<!-- [AUTH-TOKEN-VERIFY] -->`

**CSS comments** (`site/src/**/*.css`):
- `/* Implements [AUTH-TOKEN-VERIFY] */`
...
Search test files for the spec ID. Test files in this repo are found in:

- Rust: `crates/*/tests/**/*.rs`, `crates/*/src/**/*.rs` (inline `#[cfg(test)]`) — E2E integration tests live in `crates/*/tests/`.
- TypeScript (VSIX extension): `clients/vscode/src/**/*.test.ts`, `clients/vscode/test/**/*.ts`, `.vscode-test*.mjs` driver files.
- Any fixture directories under `crates/*/tests/fixtures/` or similar.

Use Grep to search these locations for the literal spec ID string.

Tests should contain the spec ID in comments, test names, or annotations. Relevant frameworks for this repo:

**Rust** (`cargo test`, `assert_cmd`, E2E black-box):
- `// Tests [AUTH-TOKEN-VERIFY]`
- `#[test] // Tests [AUTH-TOKEN-VERIFY]`
- `#[test]` `fn test_auth_token_verify_flow() { ... }` where the function name embeds the ID semantics
- Doc-comment on the test function referencing the ID

**TypeScript** (Mocha/VS Code test runner for the VSIX):
- `// Tests [AUTH-TOKEN-VERIFY]`
- `describe('[AUTH-TOKEN-VERIFY] description', () => ...)`
- `test('[AUTH-TOKEN-VERIFY] should ...', () => ...)`
- `it('[AUTH-TOKEN-VERIFY] ...', () => ...)`
...
5. **Find test refs:** Grep for the literal spec ID in test directories and test file patterns (`crates/*/tests/`, `clients/vscode/src/**/*.test.ts`)
