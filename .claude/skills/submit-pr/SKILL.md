...
3. **Adversarially review the diff.** Attack it, don't bless it. Work every lens below, each finding as `file:line` + a concrete failure scenario. Refute what you can, fix what survives, re-run steps 1–2. Hard gate — no PR with a finding outstanding.
   - **Correctness** — error paths, byte-offset/UTF-8 boundaries, empty repo, CRLF/non-ASCII, score ties and overflow.
   - **Duplication** — new code units through MCP `find-similar`; `fused ≥ 0.85` blocks. MCP down → STOP, `gh issue create`.
   - **Hard rules** — file/function size, `unwrap`/`panic!`/`unsafe`, suppressions, missing `///`, `println!`, state outside `state.rs`, regex on source, hand-written wire structs.
   - **Tests** — name the test that fails if you revert each change. Flag deleted tests, softened assertions, `sleep`/random, fake LSP/MCP, lowered thresholds.
...
6. **Call out breaking changes explicitly** in the Details section if any public API, CLI flag, report format, or spec ID changed.
7. **Create the PR** with `gh pr create --base main --title "<title>" --body-file <path>` (write the filled template to a temp file and pass it via `--body-file` to preserve formatting).
8. **Monitor CI on the PR until it is green — and re-run the suite locally *in parallel* so you catch breakage early.** This step is mandatory and does not end until every required check on the PR has passed. Do not hand the PR back to the user on a red or still-running pipeline.
   ...
   - **Run step 3 over every fix diff before pushing it.** Rushed CI fixes are where defects slip in.
...
- **Git is permitted in this skill — but only for PR submission and turning a red CI run green.** This is the one place the repo-wide "no git" rule from [CLAUDE.md](../../../CLAUDE.md) is relaxed: you may run `git add`, `git commit`, and `git push` to land CI fixes (steps 7 and 8). Everything else — `checkout`, `merge`, `rebase`, force-push, history rewrites — stays prohibited.
...
- Every step-3 lens attacked; findings refuted or fixed.
