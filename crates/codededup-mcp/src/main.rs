//! `codededup-mcp` binary — MCP server over stdio.
//!
//! Thin shell over [`codededup_mcp::McpServer`]: parse CLI args,
...
use codededup_mcp::{McpServer, PipelineSessionBackend, SessionBackendConfig};
use tracing::error;
use tracing_subscriber::EnvFilter;

/// `codededup-mcp` — MCP server over stdio exposing live clone
...
fn install_tracing() {
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("codededup_mcp=info,codededup_core=info"));
