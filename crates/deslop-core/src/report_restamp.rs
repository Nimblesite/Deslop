...
//!
//! Rank, severity band, shape reading, occurrence count, and the
//! evidence sentence are all *computed* values. The accuracy contract
//! puts every such computation in the engine and carries the result on
//! the wire, so no consumer re-derives a figure the report already
//! states. That obligation has two entry points and this module owns
//! both: the render path stamps a freshly built cluster, and
//! `--from-report` restamps a report read back from disk, which may
