...
/// 🛑 **QUARANTINED — accuracy defect. Do not restore without a fix.**
///
/// What this did: it scanned the first kilobyte of a file's raw bytes for
/// the markers `@generated`, `generated code`, `auto generated`,
/// `auto-generated`, `autogenerated` and `automatically generated`,
/// ASCII-case-insensitively. Any hit hid the entire file — every
/// occurrence was flagged `hidden` at render time
/// ([`crate::report_render`]) and every line of it was struck out of
/// `duplicated_loc` and `duplication_percent`
/// ([`crate::report_metrics`]).
///
/// Why it was removed: it matched raw bytes, so it could not tell a
/// generator's banner from prose *about* generators. A file's header
/// comment is exactly where a human writes "these bundles are
/// automatically generated", and a header comment sits inside the first
/// kilobyte, so the mitigation this function claimed — scanning only the
/// head so "a stray phrase deep in hand-written source cannot trip it" —
/// never covered the likeliest case. Two byte-identical hand-written Dart
/// files were clustered, then suppressed, and reported as 0.0%
/// duplicated: a false negative in the ranked report and in the headline
/// percentage at once. This repository holds a live instance —
/// `crates/deslop/tests/issue_165_dart_generated_header.rs` is hidden from
/// its own report because its module comment quotes the Flutter banner,
/// which drops 39 duplicated lines out of `duplicated_loc`.
///
/// The behaviour is also unspecified: [EXCLUSION-CONFIG]
/// (`docs/specs/exclusion.md`) defines `report_hide` as path patterns,
/// filename suffixes and path components, and says nothing about
/// inspecting file content.
///
/// Pinned by
/// `prose_about_generators_does_not_hide_hand_written_duplication` in
/// `crates/deslop/tests/issue_165_dart_generated_header.rs`.
#[must_use]
#[allow(
    clippy::panic,
    reason = "accuracy quarantine: a crash is recoverable, a silent false negative is not"
)]
pub(crate) fn has_generated_header(_source: &[u8]) -> bool {
    panic!(
        "has_generated_header was quarantined: byte-matching a generated banner in \
         the first kilobyte hid hand-written files that merely describe generators, \
         erasing real duplicates from the report and from duplication_percent. See \
         prose_about_generators_does_not_hide_hand_written_duplication."
    )
}
