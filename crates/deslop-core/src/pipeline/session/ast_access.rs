...
//! session's in-memory state (`per_file` / `sources`) and are exposed
//! here as borrows — **never serialised to the wire**
...
impl PipelineSession {
    ...
    pub fn subtree_at_range(&self, file_id: FileId, range: ByteRange) -> Option<&NormalizedNode> {
        self.per_file
            .get(&file_id)
            .and_then(|cached| cached.tree.smallest_covering(range))
    }
}
