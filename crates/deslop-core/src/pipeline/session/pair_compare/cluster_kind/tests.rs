...
fn node(kind: &'static str, file_id: FileId, range: ByteRange, children: Vec<NormalizedNode>) -> NormalizedNode {
    NormalizedNode { kind, children, byte_range: range, file_id }
}

fn partitioned_tree(file_id: FileId, divide: usize) -> NormalizedNode {
    let range = ByteRange { start: START, end: SOURCE.len() };
    let first = node(IDENTIFIER_KIND, file_id, ByteRange { start: START, end: divide }, Vec::new());
    let second = node(IDENTIFIER_KIND, file_id, ByteRange { start: divide, end: SOURCE.len() }, Vec::new());
...
fn equal_hash_and_bytes_can_have_disagreeing_content_frontiers() {
    ...
    let scope = PairScope { same_file: false, interior: false, core: false };
    let evidence = pair.whole(&sources, scope);
    assert!(pair.resolved(), "both source frontiers resolve");
    assert!(evidence.support() < CONTENT_SUPPORT_FLOOR, "authored leaf content differs");
    assert_eq!(exact_text_kind(&left, &right, &sources, &languages, || pair.resolved()), None);
}
