...
//! (tool version, group counts, embedding provenance, schema doc) is
//! tucked into a single
...
use crate::{
    ...
    render::{
        ...
        html_snippets::{render_snippet_body, SnippetLoader},
    },
...
fn intro_summary(report: &Report) -> String {
    let groups = report.metrics.clusters_total;
    ...
    sentence.push_str(
        "Clones are ranked by duplicated mass. Shape-only findings are informational and listed last.",
    );
    if hidden > 0 {
...
fn write_clusters_flat(out: &mut String, report: &Report, snippets: &mut SnippetLoader<'_>) {
    let _ = write!(out, "<section><h2>Findings</h2>");
    if report.clusters.is_empty() {
