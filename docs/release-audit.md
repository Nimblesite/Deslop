...
- **The VS Code extension host has no line-coverage floor (#440).** `coverage-thresholds.json` carries `vsix.webview_threshold: 95` and nothing for `out/**`; the desktop extension host ignores `NODE_V8_COVERAGE` for plain-Mocha suites. This is the file's one sanctioned exception and it is documented as such, but it is still ~6,300 lines of `clients/vscode` changed this cycle under no gate. Migrate the suites to the Testing API and restore a floor.
