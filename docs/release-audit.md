...
- **Spec-vs-test conflict (stop item) — gh #462.** `verbatim_subgroup_survives_noise.rs::two_identical_collection_cells` asserts an intra-file byte-identical pair **must publish** — the negation of the decided [CLONE-NOISE-VERBATIM-SUBGROUP-CROSS-FILE]. Red at HEAD; it was green until the cross-file rule landed, so it is a **fifth #434 casualty**. The fix is decided (`docs/specs/noise.md`: stage the proven copy across files); ownership between `release-gate` and `pi-audit` under negotiation.
- **gh #464 — sub-gate clusters still bill the metric.** The #72 trio carries `meets_fused_gate=false` (fused 0.57 against the 0.85 gate) yet contributes 15 of 31 `duplicated_loc` and drives 67.39%. The tool says it is not confident, then bills the metric anyway.
...
- **gh #460 — systemic false positive, scale measured.** 497 of 1306 visible clusters (38%) sit in `identical`/`nearly_identical` buckets with support below the 0.7 floor and no semantic support — this repo's own #1 cluster rides it at agreement 0.000.
...
- **gh #412 (back) — the corpus gate runs ZERO tests and exits 0.** `Makefile:428` sends prefixed test names into `--exact` (real names carry no prefix); `corpus.yml:83` matches on a substring. A gate that selects nothing proves nothing; a fix agent is on the selector.
...
- [ ] **#434 — fix #72 / #107** — **owner `release-gate`, in progress**, implementing [CLONE-NOISE-VERBATIM-SUBGROUP-CROSS-FILE] and [-EXACT-BYTES] exactly as `docs/specs/noise.md` states. **Fresh:** #107 **passes under `--ignored`** at HEAD — EXACT-BYTES works; only its stale ignore reason blocks it. #72 still fails — but the trio publishes via the **`structural_only` route**, not the hatch. Unticked.
...
- [ ] **Strict `make test-corpus` — a signable either/or, corrected** — **owner `release-gate`.** The gate currently proves nothing: it selects **zero tests** and exits 0 (gh #412 selector bug, back — fix agent on it). #428 leaves an **un-baselined false negative on the tokio corpus** open. #166 (runner memory) and #426 (manifest contract) are **infrastructure**, shippable-around. Either land the selector fix and re-run, or ship with this exact release-note sentence: *"This release ships without a corpus run: the corpus gate ran empty — zero tests selected, gh #412 — and gh #428 leaves an un-baselined false negative on the tokio corpus open; gh #166 and gh #426 are infrastructure defects this release ships around."* (An earlier draft claiming "every other gate ran green" was false twice and must not ship.)
