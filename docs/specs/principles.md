...
- **The watcher → scheduler → pipeline path is reactive.** The `notify`-backed file watcher feeds the [LIVE-WATCHER] / [LIVE-SCHEDULER] debouncer, which calls `update_files` and produces a new report generation. No surface polls — the pipeline pushes.
- **The LSP wire is reactive.** The session fires `deslop/reportChanged` ([LIVE-NOTIFICATIONS]) for every observable change, removals included. There is no `pollReport` request and there will never be one.
- **The MCP wire is reactive.** Agents see the same fresh report through the same notification channel; an agent that queries `find-similar` and reads the result one tick later is reading a snapshot consistent with the file system, not a cached one.
...
- **Library core.** `deslop-core` owns the pipeline. The CLI is a thin shell. An MCP/LSP binary is just a second shell over the same crate.
