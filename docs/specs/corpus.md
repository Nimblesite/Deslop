...
- Language-agnostic: a top-ranked cluster whose first occurrence is a **table of literals** — a collection whose every element is a literal, or a run of at least three declarations whose every value is one — is a data table ranked as logic, and the gate fails. The verdict is read from the occurrence's parse tree, never from characters of source text: the rule that counted digits and separators scored a table of state names 0.00 and ordinary arithmetic under a version-matrix comment 0.74 (gh #452). A span that calls anything is code however many literals it holds, and a language with no curated table grammar fails the gate rather than passing it, for the same reason as the heritage rule above. Any visible closure component keeps its full mass and carries no data category. Implemented in `crates/deslop-test-support/src/corpus_data_table.rs`, with its per-language grammars beside it; every corpus language is asserted in both directions by its tests.
...
**One predicate, both directions.** An entry is *matched* when some published cluster shows a distinct visible source location overlapping each listed range. One broad occurrence spanning both ranges, or two identical records of that location, do not show a pair. Occurrences with missing or inverted line coordinates cannot match. A matched CLEARLY IN is correct and an unmatched one is a false negative; a matched CLEARLY OUT is a false positive and an unmatched one is correct. Overlap rather than exact line equality is what makes the assertion non-fragile — it survives extent drift, rank movement, band movement and occurrence-count changes, and breaks only when the engine stops pairing two regions it must pair, or starts pairing two it must not.
...
The protocol lives in two skills, deliberately split so neither role can drift into the other: `.agents/skills/clone-register-prepare` builds the workspace and files what comes back, and `.agents/skills/judge-clone-pairs` is the judging protocol, which never names this project and is copied to the root of the handed-over folder as `JUDGING.md`. Each repository directory links to that file, so a judge never reaches back here to read it. The A/B key and the pinned checkouts are written beside that folder and never inside it.
...
- **The judging protocol**, copied to the root as `JUDGING.md`, with a root `README.md` and a link named `JUDGING.md` in each repository directory. The root holds only those two files and the repository directories. Each repository holds only its source, reports, candidates, pinned commit, verdicts, and protocol link. This exact layout is required by the protocol before any verdict is made.

Four rules make the folder worth having, and each is enforced rather than trusted:

0. **Every word the judge reads is a copy.** The protocol and guide are files in `.agents/skills/judge-clone-pairs/`, copied across byte for byte — the protocol from `SKILL.md`, the guide from `handover/root/README.md`. Nothing is composed while the folder is built: prose written at run time is prose nobody reviewed, and two passes a month apart would hand two different folders to two judges.
...
**A pair is imported only when every source agrees on it** — this repository's existing registers and all the judged folders together. Each supplied judging folder must be distinct and file exactly one ruling on that candidate, including the source ranges it saw. A missing or repeated ruling leaves the pair out, as does a ruling without those ranges. Three sources agreeing with a verdict the register already holds leave it exactly as it is; two judged folders agreeing on a pair the register does not hold add it. Anything else is left out and written to the report. There is no majority, no tie-break and no preferred judge: a pair two readers see differently is exactly the pair a register must assert nothing about.
...
- `incomplete_independent_passes` — one or more supplied folders omitted the pair or repeated its ruling; no verdict from that candidate is imported.
...
- **Judges must finish the same repositories.** A repository absent from one folder is an unfinished pass, not an empty verdict. The merge refuses the whole run before creating or draining any register.
...
**Matched-range coverage is description.** For each judged range, the scorer counts the union of its lines overlapped by visible occurrences of a matching cluster; it sums those covered lines and the inclusive judged lines across the ranges. When several clusters match, it shows the one covering the most judged lines, retaining report order for a tie. `score.json` records both counts and their percentage for each matched entry, and `SCORE.md` shows them beside each engine's verdict. The corpus standing also sums covered and judged lines across reported CLEARLY IN pairs, excluding missed and CLEARLY OUT pairs; `score.json` carries the same aggregate. A one-line overlap remains a match, but the reader can see that it covers little of the judged source. Coverage never changes a verdict or gate. Implemented in `crates/deslop-test-support/src/corpus_score/coverage.rs` and `gate.rs`, and asserted by `the_scorecard_describes_how_much_of_each_pair_was_reported` under [CORPUS-SCORE-RENDER].
...
**[CORPUS-SCORE-COST-COMPLETE] An incomplete measurement is absent, never fast.** Aggregate wall time is shown only when every scored repository has a timing file. Aggregate CPU time and peak memory additionally require that every such file measured that field. The JSON scorecard records an absent aggregate as `null`, and the Markdown shows `—`; neither turns a missing run into zero cost. The strict score gate fails when a judged run lacks its timing file, while a comparison can still render the available per-repository measurements without publishing a partial aggregate.
...
**The scorecard states its own scope.** Before any figure, `SCORE.md` says how many repositories were scanned, how many distinct languages they cover, and names those languages. A run over part of the corpus is the normal case — the whole corpus takes far longer than anyone waits — so a scorecard that opens on a score invites a partial run to be read as a full one. This is the same reason [CORPUS-CI] makes a scheduled run name the repositories it skipped. Rendered by `[CORPUS-SCORE-RENDER]` in `crates/deslop-test-support/src/corpus_score/render.rs`; asserted by `the_scorecard_states_how_many_repositories_and_languages_it_covers`, which counts three repositories across two languages so a count cannot be right by accident.
...
### [CORPUS-SCORE-GATE-SLICE] The default gate covers judged languages

The default `score-gate.sh` slice includes Polly and FSharp.Data so every push checks judged C# and F# pairs alongside the existing Python, Go, JavaScript and Rust registers. `--list-default-targets` prints each target's URL, exact judged commit and language directly from its register without building or scanning. `scripts/repository/score-gate-source.test.mjs` checks both resolved targets and keeps the [CORPUS-REGISTER-COVERAGE] floor of 15 judged pairs for every slice member. FSharp.Data cleared that floor through two independent source-only judging passes; only consensus verdicts entered its register, while disagreements stayed out of the gate. A language with fewer firm verdicts needs more independent judging before it enters the default gate; lowering the floor would make an accuracy claim rest on too little evidence.
