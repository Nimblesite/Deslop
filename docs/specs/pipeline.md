...
The grades are only comparable between views of comparable scope. **Inside one authored declaration they are not**: a window nested in the occurrence it competes with scores a higher edge exactly to the extent that it drops the statements the two copies disagree on, so ranking by grade there elects whichever window omits the most. Measured on `typescript-type3`, where `accumulate` and `aggregate` differ by one trailing statement: the enclosing view of the two functions measured 0.857 and the 37-node `let` + `for` run nested inside it — the extra `running = running + 2` cut off the end — measured exactly 1.00. The run won, the pair published as Merkle-equal, and the one statement that makes it a Type-3 near-miss disappeared from the report (#408). This is the same non-comparability [PIPELINE-CLUSTER-SUBSUME] already resolves between nesting *cluster* views, one stage earlier and with no content evidence yet measured.

So: **when two occurrences of one run sit strictly inside the same authored declaration and one encloses the other, the enclosing one is published** and the grades are not consulted. Across declarations the two spans describe genuinely different amounts of authored code, the grades are comparable, and the strength contest stands unchanged.

Nothing here is a threshold, and no threshold would do. The two cases invert on every scalar available: the encloser must **win** at 0.857 in `typescript-type3` and must **lose** at 0.879 in `fsharp_issue_339_sibling_window_rename`, where the shared window spans two top-level `let` bindings whose tails differ in shape and no function production encloses either view. Node-mass share (0.755 against 0.798) and byte share (0.747 against 0.816) invert with it. Scope decides because scope is the thing that actually differs.
