...
Behaviour-bearing anonymous tokens are therefore kept, collapsed to one `__op__` kind exactly as identifiers collapse to `__ident__` and literals to `__literal__`. Collapsing them to a single kind is what keeps Type-2 recall intact: a consistently renamed clone still fingerprints identically, and an operator family still clusters as a candidate. What the placeholder adds is a **position on the content frontier** whose raw bytes are the operator itself, so `+` and `-` disagree there and [FUSION-CONTENT-GATE] prices the difference instead of certifying it.
...
The same predicate is what `cluster_filters::body_shape` compares bodies by, so the signature-only and polymorphic suppressions no longer answer "these bodies are the same" about implementations that compute different answers. Pinned by `crates/deslop/tests/operator_drift_is_not_duplication.rs`.
...
Only a component spanning **two or more regions** is split, into one cluster per region. Such a component is a merge, not an N-way clone, and reporting the regions separately publishes strictly more true duplication than reporting their union — which is published as nothing at all.
