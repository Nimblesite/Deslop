...
**A view overturned after it has absorbed others releases them.** A view absorbs its nested rivals as the scan walks past them and only later meets the rival that overturns it. Those absorbed views are restored rather than dying with their absorber, and are re-judged against whatever survived — otherwise nothing reports their bytes at all. Honest `structural` ([FUSED-SHARED-SUBTREE](fused.md#fused-shared-subtree)) made this routine rather than rare: a whole-file view is now admitted, absorbs the genuine method-level view, and is then overturned by a single verbatim core nested inside it, so `javascript-type3` reported a byte-equal loop body in place of the near-identical function enclosing it. Pinned by `js_ts_clone_buckets.rs`.
...
Ranking uses the duplicated-mass formula owned by [RANK-MASS-SUM] below. Clusters sort by weight descending and cluster id ascending. Visible member count excludes [EXCLUSION-CONFIG] `report_hide` occurrences; category and structural-only policy multipliers apply, but fused confidence and spanned LOC do not.
