...
**Which view survives?** File coverage first, then measured content credibility, then physical enclosure, then precision. Content evidence is attached to every materialised cluster *before* this choice runs ([REPAIR-SUBSUME-CONTENT-FIRST], #367, #408): subsumption deletes whole views, and a survivor elected on raw geometry cannot be re-elected later. Pinned by `content_proven_nested_clone_survives_content_poor_enclosing_view` in `crates/deslop/tests/cross_cluster_collapse.rs` and by `crates/deslop/tests/type3_enclosing_method.rs` — whose C# case is live and gating. That suite's other four languages are `#[ignore]`d on gh #408 and are **not** a counterexample to this section: their enclosing method pair is never *admitted* by `pair::survival_decision` in the first place, so no election order could have kept it. That is a candidate-admission defect, tracked separately.
