...
#### [PIPELINE-CLUSTER-EXACT-ADJACENT] A byte-identical run between unrelated neighbours stays whole

Two files can share an identical run of adjacent statements while each wraps it in code of its own: one opens on an interface and closes on an exported constant, the other opens on a function and closes on a class. The whole files are near-copies only because the run dominates them, and the extra rows at each end are not duplication. The synthetic file root is therefore read like a window over its top-level declarations for [PIPELINE-CLUSTER-EXACT-SCOPE-MATCHED]: a root whose first or last declaration has no kind in the other copy is unmatched, and the matched run is the finding at its full extent. A module copied whole still opens and closes on the same kinds in every copy, so it stays matched and keeps its whole-file view. Implemented in `cluster/matched.rs` (`top_level`). Pinned by `clone_extent_recovery::adjacent_calls::identical_adjacent_calls_keep_their_full_extent` (TypeScript: seven identical `axios` calls and their handler publish as one identical clone at lines 2-32 in both files, rank 1).
...
Two components describe the same physical duplication only when their occurrences pair by containment in the same file. Either they pair one to one, with each copy of one containing, or contained by, one distinct copy of the other. Or one reads the other at a coarser grain: every copy of the wider view holds the same number of the other's copies, and each of those copies sits in exactly one of them. A broad window that holds two copies of an identical setup while its partner holds one cannot stand in for them. A shape-only view reports no duplication, so a wider view that strictly encloses it, with some of it inside each of its own copies, re-describes it however unevenly ([CLONE-BUCKETS-STRUCTURAL-ONLY](taxonomy.md#clone-buckets-structural-only-shape-only-is-not-duplication)). Bare intersection and one-way coverage are insufficient. A view naming a file the other does not name is never dropped. An exact fragment nested inside an edited enclosing clone is the same duplication read narrower, so the enclosing view survives it.

When two components describe the same duplication, the survivor is selected by file coverage, physical enclosure, occurrence coverage, duplicated mass, and stable cluster-id tie-breaking in that order. Structural, Jaccard, embedding, content, rename, literal, pair classification, and any presentation label are forbidden inputs to that survivor order. A view is published when no published view describes the same duplication and outranks it; every other view is absorbed by one that does. That is a property of the published set, not of the order views were met in, so whatever a view absorbed is judged again against the views that remain when it leaves the report ([PIPELINE-CLUSTER-SUBSUME-KERNEL]).

Pinned by the one-to-one, even-holding and split-and-merge cases in `crates/deslop-core/tests/cluster_subsumption/region.rs` and the three repeated C# transitions in `crates/deslop/tests/cluster_extent_alignment/same_file_tail.rs`. Assertions must prove the survivor's files, occurrences, ranges, and mass; pair scores cannot be asserted on a cluster.
...
Implemented in `cluster/exact_runs.rs` and `cluster/subsume/kernel.rs`; pinned by `copied_straddling_windows_are_not_discarded_without_a_join`, `copied_overhang_keeps_its_full_window`, `two_windows_straddling_one_nested_view_publish_that_view`, `a_view_that_yielded_to_a_straddler_is_released_when_it_dies` and `a_view_nested_in_only_one_straddler_leaves_both_published` in `crates/deslop-core/tests/cluster_subsumption/straddle.rs`, and by `cross_cluster_collapse::{long_verbatim_statement_run_is_one_complete_clone,padded_windows_straddling_a_verbatim_block_publish_the_block}` end to end.
