...
- **A demoted view yields only to verbatim-proven duplication.** The reverse of the rule above is deliberately narrower: any sub-window of a demoted surface measures higher agreement than the surface itself — the divergent positions are what the window excludes — so an unconditional "credible beats demoted" let the #197 in-file sibling family resurface as a credible six-line window family the moment its demoted umbrella died (`dart_issue_197_single_file_structural_only`). A demoted enclosing view therefore still absorbs its own diluted sub-views, and is overturned only by a nested view that is **token-identical across files** — a single family of members sharing both the same normalised-subtree digest and the same collapsed-leaf keys, holding a strict majority of the view (`ContentEvidence::verbatim_dominated`), whose occurrences reach at least two files. Token-identical copies are proof of duplication in their own right (the collapse pin's inner block), and narrowing a demoted surface must not launder it into a finding. Both halves of that test are load-bearing: a bare majority share let two disjoint pairs at exactly one half certify each other, and a same-file overturn let the #197 in-class sibling family narrow its way back to a finding.
