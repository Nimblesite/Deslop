...
Two components describe the same physical duplication only when their occurrences pair one to one by containment in the same file: each copy of one contains, or is contained by, one distinct copy of the other. A broad window spanning two later methods cannot stand in for both copies of their identical setup. Bare intersection and one-way coverage are insufficient. A view naming a file the other does not name is never dropped.
...
#### [PIPELINE-CLUSTER-SUBSUME-KIND] An exact copied core survives a broader edited clone
An exact Type I copy nested inside a broader edited clone is a distinct finding even when their occurrences line up one to one. The broader view reports changed code as a near miss; it cannot report the byte-identical extent or its stronger classification. Both survive. Two nested views of the same clone kind still collapse by the ordinary survivor order. The three repeated C# circuit setups and their three edited enclosing methods in `same_file_setup_keeps_every_fault_assertion_tail` pin both findings and their ranks.

Pinned by the split-and-merge cases in `crates/deslop-core/tests/cluster_subsumption/region.rs` and the three repeated C# transitions in `crates/deslop/tests/cluster_extent_alignment/same_file_tail.rs`. Assertions must prove the survivor's files, occurrences, ranges, and mass; pair scores cannot be asserted on a cluster.
