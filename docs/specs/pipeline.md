...
**Format.** `u32` magic, then a recursive `NormalizedNode` tree (`u32 kind_len`, kind UTF-8 bytes, `u64 start`, `u64 end`, `u32 child_count`, children...), then `u64 fingerprint_count` followed by one `{ [u8;32] hash, u64 start, u64 end, u64 node_count }` record per fingerprint, then `u64 signature_count` followed by one 128×`u64` MinHash signature per fingerprint, positionally 1:1 with the fingerprint records. Decode rejects any blob whose signature count disagrees with its fingerprint count. No serde, no schema drift: the magic + tool-version path segment bracket every format change — the pre-signature layout's magic now decodes as a plain miss and its blob is rewritten in the current format.
