...
Determinism holds across *platforms* too, and that starts with how a path is written. Every path a report renders — occurrence, `metrics.per_file`, `metrics.folders`, boilerplate row, and the path a stale-diff refusal names — is scan-root-relative and joined with `/` on every platform. A report path is a wire value: the corpus manifests, the VSIX links, the MCP `path_contains` filter and every AI consumer name a file that way, so serialising the platform separator made one tree render two different reports and made every consumer's path comparison platform-conditional — `metrics.folders` had always joined with `/`, so a Windows report shipped both conventions at once. `paths::wire_path` is the single place the form is produced, reached through `report_render::relative_to_scan_root`, and the rewrite applies only where the platform separator is not `/` — a POSIX file name may legally contain a backslash, so POSIX paths pass through untouched. Pinned cross-platform by `deslop_core::paths::tests`, which supply the separator explicitly rather than reading the running platform's, and end to end by `location_rendering::every_reported_path_is_joined_with_the_wire_separator` (gh #439).
