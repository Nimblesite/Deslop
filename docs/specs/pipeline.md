...
One duplication is fingerprinted at many depths, so a cluster carries several overlapping occurrences of the same region in the same file. The same-file overlap collapse reduces each overlapping run to one published occurrence, and it ranks the candidates by the **strongest cross-file discovery edge** each carries, width breaking ties. That ordering is what stops a weakly token-matched wider view displacing an exact sibling window (#339).

The grades are only comparable between views of comparable scope. **Inside one authored declaration they are not**: a window nested in the occurrence it competes with scores a higher edge exactly to the extent that it drops the statements the two copies disagree on, so ranking by grade there elects whichever window omits the most. Measured on `typescript-type3` and `javascript-type3`, where `accumulate` and `aggregate` differ by one trailing statement: the enclosing view of the two functions measured 0.857 and the 37-node `let` + `for` run nested inside it — the extra `running = running + 2` cut off the end — measured exactly 1.00. The run won, the pair published as Merkle-equal, and the one statement that makes it a Type-3 near-miss disappeared from the report (#408). This is the same non-comparability [PIPELINE-CLUSTER-SUBSUME] already resolves between nesting *cluster* views, one stage earlier and with no content evidence yet measured.

So: **when one occurrence of a run encloses another that sits strictly inside an authored declaration, and the encloser sits inside that same declaration or inside none at all, the enclosing one is published** and the grades are not consulted. The second half of that is load-bearing. The view a Type-3 near-miss wants published is frequently the declaration *itself*, or the `export` statement the grammar wraps around it, and the scope resolver reports no enclosing declaration for those by design — a view that is the declaration is not inside it. Requiring both sides to sit strictly *inside* one declaration therefore left the whole-declaration view unprotected, and any Merkle-equal window inside it displaced it on grade. `javascript-type3` published the nested `let` + `for` run at 1.00 over the `accumulate`/`aggregate` pair at 0.868 for exactly that reason, off the same source shape `typescript-type3` gets right — TypeScript escaped only because its type annotations put a fingerprint boundary strictly inside the declaration, an accident of that grammar rather than a rule (#427). A candidate that sits inside **no** declaration never triggers the guard, whichever side it falls on. Across declarations the two spans describe genuinely different amounts of authored code, the grades are comparable, and the strength contest stands unchanged.

Nothing here is a threshold, and no threshold would do. The two cases invert on every scalar available: the encloser must **win** at 0.857 in `typescript-type3` and at 0.868 in `javascript-type3`, and must **lose** at 0.879 in `fsharp_issue_339_sibling_window_rename`, where the shared window spans two top-level `let` bindings whose tails differ in shape and no function production encloses either view — so the window's own scope is `None` and the grade contest correctly decides it. Node-mass share (0.755 against 0.798) and byte share (0.747 against 0.816) invert with it. Scope decides because scope is the thing that actually differs.

Declarations are read off the normalised tree and keyed by the file's language, so a production name meaning "function" in one grammar cannot classify a node in another. A view that *is* the declaration is not inside it — the enclosure must be strict — so an exact whole-function occurrence and a window nested in it remain views of different scopes, with the declaration as the wider one.

Pinned by `typescript_near_miss_produces_cross_file_structural_cluster` in `crates/deslop/tests/js_ts_signatures.rs` and `javascript_near_miss_extra_guard_is_a_proven_rename` in `crates/deslop/tests/js_ts_clone_buckets.rs`, which fail the moment either axis saturates over members a real subtree separates, and in the other direction by `crates/deslop/tests/fsharp_issue_339_sibling_window_rename.rs`.
...
The component that results is a clone of nothing. Its members do not agree, so [FUSED-CONTENT-GATE] measures a low `agreement` and a low `rename_consistency` over the union, the cluster buckets down to `loosely_similar`, and report policy hides it. Both real families are then lost *to the presence of each other*, and the loss grows with the corpus: the more code a scan reaches, the more token bridges it finds. This is the direction that matters — a defect that gets worse on real repositories and cannot get worse on a fixture.

The rule holds inside **one language** and is not applied outside it. Two normalised subtrees of one grammar differ because the code differs; two subtrees of *different* grammars differ because the grammars do, and nothing follows about whether either is a copy of the other. A component spanning languages exists only because [CONFIG-CROSS-LANGUAGE](exclusion.md#config-cross-language) was opted into, so splitting it into one cluster per language would delete precisely the finding that opt-in asks for. Such components are left whole, as is any component whose languages cannot be resolved.

Families of **two or more members** are therefore taken as the unit, before any signal is measured, and the members belonging to none are dropped. Splitting on the digest is safe in the direction that matters: two distinct hashes mean two distinct normalised subtrees, and no rename and no literal edit can produce that, because normalisation collapses exactly those.

But a family is not yet a finding, because one duplication is fingerprinted at many depths and each depth is its own family. Families are first merged into the **regions** they cover: two families describe one region when every occurrence of each shares bytes with some occurrence of the other. That is [PIPELINE-CLUSTER-SUBSUME]'s own coverage question, asked here so this pass never answers it — a copied method and the statement run inside it stay in one cluster, where the same-file overlap collapse and subsumption elect between the views on discovery evidence this pass has already discarded. Splitting them would publish both, show one duplicate as two findings, count its lines twice, and strand the byte-proven view against a wider near-miss that outranks it across clusters.

One-way coverage is not a nesting, and this is what makes the pass work at all. A shallow shape duplicated across four files encloses a two-file clone in two of them and covers code that clone never reaches in the other two, so neither is a view of the other and both are real findings. Such a bridge is exactly what welds two disjoint clones into one component, and merging only on *mutual* coverage leaves it in its own region rather than gluing the corpus back together.

But a region count is not a weld. Mutual coverage also fails one way when an enclosing view reaches **fewer** files than the run nested inside it — a whole-module shape shared by two files, and the function shape inside it that also appears in a third. Nothing was welded there: it is one piece of code fingerprinted at two depths over an uneven file reach. Splitting it publishes the enclosing view as a cluster of its own, narrower in members and files than the family it came from, and so beneath the spread floors that were suppressing the pattern — a false positive manufactured out of a finding the report had correctly withheld. The #112 pytest payload fixtures reached the report exactly that way, as a whole-module pair published `structural_only` at `agreement = 0.14`.

A component is therefore split only when it spans **two or more regions** *and* two of those regions **share no byte**. Disjoint bytes are what make two regions distinct code, and distinct code is what a token edge welds. The bridge case is unaffected: the two clones it encloses share no byte with each other, so the weld is present and all three regions publish — the bridge touching both is what made it a bridge, not a concealment of the weld between them. A component whose regions all touch is one duplication seen at several depths, and is left whole for [PIPELINE-CLUSTER-SUBSUME] and the same-file overlap collapse to elect between.

Where a component is split, it becomes one cluster per region. Such a component is a merge, not an N-way clone, and reporting the regions separately publishes strictly more true duplication than reporting their union — which is published as nothing at all.

A component covering **one** region is left whole. An ordinary Type-3 cluster with a fringe of near-misses is one region, and its fringe members are occurrences a reader wants, so a split would delete findings rather than restore them.

The mechanism is [CLONE-NOISE-VERBATIM-SUBGROUP]'s, one layer earlier and keyed on the subtree digest instead of the source bytes; both share `cluster_filters/family.rs`. Pinned by `crates/deslop/tests/csharp_merged_clone_families.rs`, where a summing loop copied across two files and a multiplying loop copied across two more are each reported `nearly_identical` alone and, before this rule, reported **nothing at all** when scanned together, and by the unit tests in `cluster_filters/structural_families/tests.rs`, which hold the region cases the E2E cannot reach — including the four-file bridge, the nesting that must survive it, and the enclosing view whose nested run reaches a file it does not. The false positive the weld requirement removes is pinned end to end by `nested_dict_literal_fixtures_across_test_files_do_not_cluster` in `crates/deslop/tests/python_issue_112_dict_fixture.rs` ([CLONE-NOISE-PY-DICT-FIXTURE]). The three contracts it must not break are pinned by `byte_identical_clone_survives_a_demoted_enclosing_view_in_one_file` and `content_proven_nested_clone_survives_content_poor_enclosing_view` in `crates/deslop/tests/cross_cluster_collapse.rs`, and by `config_can_enable_cross_language_clusters` in `crates/deslop/tests/cross_language.rs`.

### [PIPELINE-CLUSTER-ELECT-CONTAINER] A concatenation of a family is not a finding

A token edge can also weld a structural family to the views that merely *contain* it — the class holding seven shape-identical methods, the sliding window spanning two of them, the file root over the lot. These containers do not survive to the report as clusters, but left inside the component they destroy it from within: they glue every occurrence in a file into one overlapping run, and the same-file overlap collapse then publishes the family as one container occurrence per file. Seven findings become two, the constructors, fields and imports between the methods are counted as duplicated, and both `[ranking]` orders move with the mangled weight (`rank_structural_only_policy`, [RANK-STRUCTURAL-ONLY]).
...
- the family **outnumbers** the member's own family — the container cannot express the occurrences it swallows, [PIPELINE-CLUSTER-SUBSUME]'s `nested_view_outnumbers` asked one stage earlier, where the weld would otherwise destroy the occurrence list before subsumption ever sees it;
- the family supplies at least **two thirds of the member's bytes** — [PIPELINE-CLUSTER-SUBSUME]'s `accounts_for_bulk` boundary between "this duplication plus code that is not duplicated" and "a view of a finding that is real at full extent"; an encloser most of whose bytes are its own code is the extractable view subsumption must be allowed to elect;
- every family occurrence carries the **copied-block node floor** subsumption demands before a nested view may unseat its encloser — a run of byte-equal idiom lines, four `assert` statements that are most of a small test helper, confers no container standing however much of the umbrella it covers (`python-issue-71`, `python-issue-100-kwargs-ctor`);
- the member strictly encloses **two or more** family occurrences, and the family **overflows** the member's own family — or encloses one, when the member's own family is itself reportable and the enclosed family continues past it *in the member's own file*. One enclosed occurrence with no same-file continuation is ordinary nesting: the #112 module pair whose nested run merely reaches a third file stays whole, and a lone method holding one copy of a duplicated run (`csharp-merge-readafter`) stays the merge gate's business.

The overflow test carries one forfeit: a view family whose own occurrences overlap one another in a file — sliding windows tiling a sibling row — forfeits the no-overflow exemption outright. The tiling covers every sibling between its windows, but it counts the shared siblings twice, so its coverage is not an occurrence list a report could publish, and the family it tiles is never its fine structure. A family that genuinely covers the enclosed family whole — the `fsharp-issue-339` sibling window wholly composed of two per-binding shapes — keeps its members: the window is the finding, and the shapes inside it are its fine structure.

Pinned by the container cases in `cluster_filters/structural_families/tests.rs` and end-to-end by `rank_structural_only_policy` (all seven method occurrences at whole-method extents, demoted below the byte-identical pair by default, out-ranking it under `structural_only = "keep"`) and `rename_needs_an_anchor` (the published family windows contain the endpoint literal they were previously elected to exclude).
...
**Which view survives?** File coverage first, then measured content credibility, then physical enclosure, then precision. Enclosure is nominated in **both** directions — `outer`/`inner` are weight-ordered scan positions, not nesting — and within one credibility tier the enclosing view wins outright, with no comparison of signal grades. Since `structural` became a graded overlap ([FUSED-SHARED-SUBTREE]) those grades are not comparable across scopes: a nested window scores higher exactly to the extent that it excludes what differs, so a byte-identical 28-byte parameter list scored 1.00 against its enclosing method's 0.88 and emptied the `ts-type3-stmt` report (#408). Grades decide only between views that do not nest. Content evidence is attached to every materialised cluster *before* this choice runs ([REPAIR-SUBSUME-CONTENT-FIRST], #367, #408): subsumption deletes whole views, and a survivor elected on raw geometry cannot be re-elected later. Pinned by `content_proven_nested_clone_survives_content_poor_enclosing_view` in `crates/deslop/tests/cross_cluster_collapse.rs` and by `crates/deslop/tests/type3_enclosing_method.rs`, which holds all five languages.

**A view overturned after it has absorbed others releases them.** A view absorbs its nested rivals as the scan walks past them and only later meets the rival that overturns it. Those absorbed views are restored rather than dying with their absorber, and are re-judged against whatever survived — otherwise nothing reports their bytes at all. Honest `structural` ([FUSED-SHARED-SUBTREE](fused.md#fusion-shared-subtree)) made this routine rather than rare: a whole-file view is now admitted, absorbs the genuine method-level view, and is then overturned by a single verbatim core nested inside it, so `javascript-type3` reported a byte-equal loop body in place of the near-identical function enclosing it. Pinned by `js_ts_clone_buckets.rs`.

**A byte-proven nested view that covers most of its encloser is the duplication.** Enclosure normally decides within a credibility tier. There are two exceptions, and this is the first: a nested view that is verbatim-proven across files *and* accounts for at least two thirds of the enclosing view's bytes wins, because the encloser is then that duplication plus surrounding code that is not duplicated. Both halves are load-bearing and each is pinned by a fixture the other breaks — measured, the share is 0.82 for `incremental-multilang`'s C# class-around-a-copied-method (nested wins, and the pair keeps its byte-proven `identical` label), 0.49 for `javascript-type3`'s function-around-a-copied-loop and 0.10 for `ts-type3-stmt`'s method-around-a-copied-parameter-list (encloser wins). Byte span, not node count: node mass compresses those to 0.76 against 0.63, where no threshold separates them safely.

**A nested view with more occurrences than its encloser is a family the encloser cannot express.** The second exception. A nested view holding strictly more occurrences than the view enclosing it is not re-describing that view; electing the encloser deletes the surplus findings outright and counts the code between them as duplicated. The byte-proof exception above cannot cover this case — it asks for a verbatim proof, and a shape-only family has none by construction, because normalisation strips the identifiers that differ between siblings, which is the only reason they cluster. Measured: seven shape-identical Dart API methods across two files, enclosed by a two-occurrence whole-class view scoring `structural` 0.75 against the family's Merkle-saturated 1.00 and covering 0.541 of the encloser's bytes — below the two-thirds bar, so the byte rule leaves it to die. Electing the class dropped five of the seven occurrences and reported `inventory_api.dart` 100% duplicated, 45 of 45 LOC, when only its method bodies repeat. Counting occurrences is sound where comparing grades is not: a nested window scores higher merely by excluding what differs, but it cannot invent occurrences its encloser lacks. The three fixtures bracketing the byte-proof rule all pit two occurrences against two, so this test cannot fire on them and their survivors are unchanged. Pinned by `crates/deslop/tests/rank_structural_only_policy.rs` ([RANK-STRUCTURAL-ONLY]), which asserts the family reports all seven occurrences, and by `crates/deslop/tests/cross_cluster_enclosure.rs`, which holds the opposite direction — a duplicated 60-statement method is not shattered into its nested statement views.
...
- **A view the report will demote or hide never deletes a view the report publishes as actionable.** The election reuses the renderer's own routing ([FUSED-CONTENT-GATE]) on the measured evidence, so the elected view and the rendered label cannot drift: a saturated 13-node fragment routed `structural_only` no longer overturns the whole-method Type-3 clone it nests inside.
- **A demoted view yields only to verbatim-proven duplication with real statement mass.** The reverse of the rule above is deliberately narrower: any sub-window of a demoted surface measures higher agreement than the surface itself — the divergent positions are what the window excludes — so an unconditional "credible beats demoted" let the #197 in-file sibling family resurface as a credible six-line window family the moment its demoted umbrella died (`dart_issue_197_single_file_structural_only`). A demoted enclosing view therefore still absorbs its own diluted sub-views, and is overturned only by a nested view that is **token-identical** — a single family of members sharing both the same normalised-subtree digest and the same collapsed-leaf keys, holding a strict majority of the view (`ContentEvidence::verbatim_dominated`) — whose **every occurrence carries at least `VERBATIM_OVERTURN_MIN_NODES` normalised nodes**. The mass floor is what separates a copied block from a mandatory idiom line repeating inside sibling scaffolding, measured by `--min-nodes` bisection of the deciding view: the byte-equal `assert` line of `python-issue-71-rest-endpoint-shape` and the byte-equal kwargs line of `python-issue-100-kwargs-ctor` carry 8–9 nodes per occurrence and stay absorbed — the kwargs line once deleted its umbrella and the fifteen absorbed micro-views it released were published as four visible clusters no noise filter could recognise (`message_vs_agentlog_kwargs_constructors_do_not_cluster`) — while the verbatim five-statement run inside `csharp-merge-readafter` (≥ 28 nodes, deleted by a `structural` 0.85 straddle claiming nine duplicated lines where five are) and the byte-identical `if` block shared by two otherwise-divergent functions (24–27 nodes, deleted by the whole-file echo enclosing it) overturn (`byte_identical_clone_survives_a_demoted_enclosing_view_in_one_file`, `declared_inside_read_after_refuses`, `content_proven_nested_clone_survives_content_poor_enclosing_view`). Sixteen sits between 9 and 24 with real margin on both sides, above the saturated 13-node fragment already recorded as the canonical sliver (#408). The minimum over occurrences, not the sum: a family of many tiny repeats gains no standing from its cardinality. Token-identical copies are proof of duplication in their own right (the collapse pin's inner block), and narrowing a demoted surface must not launder it into a finding — a bare majority share once let two disjoint pairs at exactly one half certify each other.
- **Between two credible views, physical enclosure stands — content never overturns it.** The *enclosing* view is the duplication and the nested view re-describes it. Ranking weight must not decide: the fine-grained view always ranks heavier because it contributes one occurrence per statement, so weight-based selection renders a duplicated 60-statement method as 120 one-line occurrences and drops the method itself. Two sharper within-credible comparisons were built, measured, and removed: raw support (1.0 against 0.89 shattered the `csharp-fact-cross-cluster` method pair into its byte-identical tail plus a straddling window) and an act-now grade over measured support (0.85 elected a verbatim core over the credible 0.8 window enclosing it and orphaned that window's other absorbed views — `issue_343_sum_clamp_saturation` counted the orphan).
...
| buckets + metrics + JSON write | ~25% |
| candidate scoring, closure, rank, content | ~5% |

Signature construction — ~69% of the LSH block before this work — is gone from the warm path entirely. Two design decisions are settled and should not be relitigated without new numbers. **Full signatures, not band hashes, are what the blob persists**: `estimate_jaccard` consumes full signatures for every candidate pair that reaches scoring and for every cluster's signal means, so persisting only the 32 band hashes per fingerprint (256 B against 1 KB) would force full-signature reconstruction for exactly the fingerprints that matter. **The recorded alternative for the banding phase** is to persist the band index (band → bucket → fingerprint hash) instead: an incremental pass evicts the changed files' fingerprints from their buckets, inserts the new ones, and reads off only the collisions involving them — O(k·N) rather than O(N²), which is what makes cost track change size. If that lands, the per-fingerprint signatures stop earning their bytes (~85% of the 185.8 MiB tokio store) and the blob can drop back to roughly its pre-signature shape.
...
in substance ([FUSED-CONTENT-GATE]). And **no two proven wrappers may share a
...
- `action_hints: Vec<ActionHint>` — short entries stating what a signal combination means ("high structural + high jaccard"). The field name predates [PRINCIPLES-REPORT-NOT-DICTATE](principles.md#principles-report-not-dictate) and is kept for wire compatibility; the contents state evidence.
...
- `id`, `weight`, `size`, `canonical_node_count`, `signals { structural, token_jaccard, embedding_cos }`, `summary` — as in v1.
...
- `clusters_total: usize` — count of non-hidden clusters carried in `clusters`, literal-family included; always equals `clusters.len()` — including after `--only-changed` filtering, where the repo-wide count is recovered as `clusters_total + clusters_outside_diff` ([METRICS-DIFF-SCOPE]) — but is carried explicitly so downstream consumers don't re-derive it. Only fragment-clone clusters contribute lines to `duplicated_loc` — [RANK-LITERAL-FAMILY] clusters are excluded from the line projection, not from this count.
...
| `bucket_weights.loosely_similar` | `0.0` | No axis corroborates the overlap ([CLONE-BUCKETS]), so it must not move a CI verdict. |
...
- No confidence-scaled percentage. A continuous multiplier makes the number irreproducible from the report; revisit only if the bucket constants prove insufficient.
