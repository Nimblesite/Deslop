...
- **A view the report will demote or hide never deletes a view the report publishes as actionable.** The election reuses the renderer's own routing ([FUSION-CONTENT-GATE]) on the measured evidence, so the elected view and the rendered label cannot drift: a saturated 13-node fragment routed `structural_only` no longer overturns the whole-method Type-3 clone it nests inside.
