...
- Header: the clone kind title ([CLONE-KIND-LABELS](taxonomy.md#clone-kind-labels-use-the-same-names-everywhere)), cluster id, rank, mass, occurrence count, a coloured badge with the visible type chip ([VSIX-CLONE-TYPE-CHIP](vsix.md#vsix-clone-type-chip-visible-clone-types)), and jump-to-next-cluster / jump-to-prev-cluster arrows.
...
The diff title names both endpoints, a compact type chip ([VSIX-CLONE-TYPE-CHIP](vsix.md#vsix-clone-type-chip-visible-clone-types)), and the engine's verdict on exactly that pair, read from `pair/compare`: `Identical bytes` when the engine's `text_identity` is `byte_identical`, `Differs only by indentation` when it is `indentation_only` (the same lines once each line's leading whitespace is removed), otherwise the classification's kind title ([CLONE-KIND-LABELS]), or the engine's explanation for a pair it does not classify. An unclassified pair has no type chip. Without a connected engine the title names the endpoints alone and claims nothing; the log says why.

Both canonical commands and the explicit `deslop.comparePair` command share the occurrence-range virtual-document provider. Same-file peers remain distinct documents and show only their saved bytes. Canonical resolution reads the current full report, so an unsaved edit does not make the cluster unresolvable ([VSIX-STATE-DIRTY]). Tree context menus mark the original canonical by identity, independent of path sorting or dirty-file projection; every surviving non-canonical peer keeps its Compare action. The detail panel gets the same guarantee through its feed rather than through a second identity check: `p` and `n` change the selection inside the webview and the host keeps no copy of that selection, so every cluster the feed lists arrives with the membership the engine published. A cluster reached by navigating therefore shows its own canonical occurrence first — never the first member that happens to be saved, which would wear the canonical label, lose its own Compare action, and hand the peer below it the wrong range to diff. The dirty projection still decides which clusters the panel lists. Code: `commands/register.ts`, `commands/compare.ts`, `compare/title.ts`, `webview/panels.ts`, `clusterSelection.ts`, `tree/providers.ts`, `store.ts`, `cluster/OccurrenceList.tsx`. Assertions: `clusters.e2e.test.ts`, `compare-canonical.unit.test.ts`, `compare-title.unit.test.ts`, `command-impls.unit.test.ts`, `panels-cluster-selection.unit.test.ts`, `package-menus.unit.test.ts`, `webview-cluster.unit.test.ts`, and `playwright-webview-smoke.spec.ts`.
