...
**Row 4 is content-gated too.** It reaches an act-now bucket on token overlap *alone*, and that overlap is measured over the normalised stream, so the same argument applies at row 4's own lower floor: six distinct Flutter widgets whose `build` bodies share nothing measure `structural = 0.00, token_jaccard = 0.93` over whole-file spans, because the framework-mandated declaration is most of each file (#331). The gate demotes them on measured content evidence while leaving a genuine reorder clone — real byte agreement — at act-now confidence. Both halves are pinned together: `crates/deslop/tests/lsh_only_nearmiss_recall.rs` asserts the genuine pair keeps `fused ≥ 0.85`, `crates/deslop/tests/issue_331_336_shape_only_saturation.rs` asserts the scaffold family does not.
