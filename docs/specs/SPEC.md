...
This doc captures formal research on code clone / duplication detection that informs CodeDedup's design. **Primary goal:** pick techniques that are (a) fast enough for a CLI to run on a whole repo, (b) accurate across Type-1 → Type-3 clones (and Type-4 where feasible), and (c) don't paint us into a corner for a future LSP mode (incremental, per-file, byte-range-addressable).
...
- **Local only, via Ollama.** No network. Source code never leaves the machine.
- **Small model.** The 2025 ensemble paper is explicit: *"smaller embedding sizes, smaller tokenizer vocabularies and tailored datasets are advantageous"*. Start with **`nomic-embed-code`** (Ollama-available, purpose-built for code, small). CodeT5+110M and UniXCoder are the alternate top performers cited in the literature — evaluate both once we have fixture repos.
...
- **Cache by `(file_content_hash, model_id, model_version)`.** Re-runs are free. LSP incremental mode reuses the cache.
...
- **Determinism caveat.** Embedding + ANN is approximate. Mitigate by: (a) pinning model version in the lockfile-style `.codededup-cache`, (b) using deterministic ANN parameters (fixed seed, fixed ef_construction), (c) final ranking is still computed over the *union* of structural + LSH + embedding candidates, so a missed ANN neighbor only loses recall, never changes existing cluster content.
...
7. **Embedding pass (local Ollama):** embed every AST subtree from step 3 with a small code embedding model (start: `nomic-embed-code`). Index via HNSW (`usearch` or `instant-distance`, both pure Rust). For each subtree, retrieve top-k neighbors above a cosine threshold. Catches Type-3 and Type-4 the prior passes miss. **This is a first-class pipeline stage, not optional.**
...
10. **Output:** ordered text + JSON report with exact byte ranges, file paths, and **per-cluster signal breakdown** (structural / LSH / embedding contributions) so users understand *why* something was flagged.
11. **Incremental cache:** `(file_content_hash) → (parse_tree, subtree_fingerprints, embeddings[model_version])`. Required for the LSP future, and makes re-runs with unchanged files free.
12. **Library vs binary split:** analysis pipeline lives in a `codededup-core` crate; `codededup` binary is a thin CLI. LSP binary later reuses the core unchanged.
13. **Offline mode:** `--no-embeddings` flag for environments without Ollama. Report will explicitly note reduced Type-4 recall. Not the default.
14. **Model pinning:** the embedding model id + version is written into the cache and into the report header. Changing models invalidates the cache (deterministic, explicit).
