...
- [fused.md](fused.md) — `[FUSED-*]` why Deslop is hybrid (not pure-RAG); embedding + ANN choices; max-sum fusion strategy; `[FUSED-TUNING-LEVERS]` the accuracy-threshold ledger — every lever, its default, and the literature, defect, or derivation that default came from.
...
| Bounded max fusion — sum quarantined by gh #343 ([FUSED-STRATEGY-BOUNDED-MAX]) | ✅ strongest single axis in `[0,1]` | `crates/deslop-core/src/pair.rs::PairScore::bounded_fused` |
