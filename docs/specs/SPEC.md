...
| Incremental fingerprint cache ([PIPELINE-INCREMENTAL]) | ✅ on by default, `--no-incremental` opts out — parse stage only, ~9% of a run | `crates/deslop-core/src/fpcache.rs` |
