...
Two entries of one collection literal, such as `"name": name` and `"arguments": arguments`, can share a shape after their names are removed. Shape alone must not admit them as clones ([CLONE-BUCKETS-STRUCTURAL-ONLY](taxonomy.md#clone-buckets-structural-only-shape-only-is-not-duplication)). A cluster is suppressed when every member's
