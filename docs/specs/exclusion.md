...
**Built-in defaults.** Without a config file, Deslop excludes common dependency/build cache directories (`node_modules`, `target`, `dist`, `build`, `.venv`, `__pycache__`) and report-hides generated output (`generated` path components plus suffixes such as `.g.cs`, `.generated.cs`, `.designer.cs`, `.pb.cs`, `.openapi.cs`). Project config adds to these defaults.
