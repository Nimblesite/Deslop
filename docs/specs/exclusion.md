...
**Built-in defaults.** Without a config file, Deslop excludes common dependency/build cache directories (`node_modules`, `target`, `dist`, `build`, `.venv`, `__pycache__`, `.cargo`) and report-hides generated output (`generated` path components, Alembic migration files under `alembic/versions`, plus suffixes such as `.g.cs`, `.generated.cs`, `.designer.cs`, `.pb.cs`, `.openapi.cs`). Project config adds to these defaults. `.cargo` blocks Cargo's vendored registry and git-checkout caches from entering discovery even when a misconfigured scan root reaches into the user's home directory.
