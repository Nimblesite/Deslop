...
Implemented in `pair/echo.rs`, applied in `overlap/rescue.rs` and `pair/content_gate.rs`; pinned by `issue_389_subsumption_modifier_straddle`, the `incremental-multilang` golden across all six languages, and `fsharp_issue_339_sibling_window_rename` (an F# module wrapping an exact two-binding window).
