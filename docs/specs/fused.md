...
Pair evidence belongs to the two occurrences being compared. Use the strongest shape or semantic signal, then check content separately. Never add or average these signals: they are not independent probabilities.
...
4. **Admission is decided pair by pair.** A pair must pass the size and token-only guards, clear its initial threshold or qualify through the shared-subtree rescue, and pass the applicable content checks. The initial score is defined in [FUSED-PRE-RESCUE-SCORE]. Group-level similarity judgement and averaging are forbidden. Clusters join admitted pairs before the separate noise rule in [CLONE-NOISE-VERBATIM-SUBGROUP].
5. Rank clusters by duplicated mass alone ([pipeline.md §RANK-MASS-SUM](pipeline.md#rank-mass-sum-rank-by-duplicated-mass-only)). No pair evidence participates in mass or order.
...
Every member of a bucket spanning files gets a cross-file candidate. An unrelated first member must not hide copies among the remaining members: compare their content, recover missing pairs within each copied subset, then apply the same admission checks. Recovery adds only structurally identical pairs; it does not lower any threshold. Pinned by `cli::category_members`.

Inside one file there is no other scope to borrow that soundness from, and which member sorts first is an accident of write order. So **members of a bucket that share a file are paired with each other, all of them.** A bucket holding one member that *differs* ahead of a byte-identical copy otherwise judged the copy only against the member that differs, and one unrelated sibling deleted an exact duplicate from the report — recall may not depend on what else happens to share the shape. No member can tell in advance which partner its content will vouch for, so within a file the bucket is completely paired. Pinned by `same_file_rescue::a_shape_sibling_may_not_hide_an_exact_same_file_copy`.
...
#### [FUSED-CANDIDATE-BUCKET-RECOVERY] Compare only pairs recovery can add

After closure, a mixed shape family may contain thousands of fingerprints with different Merkle hashes. Copy recovery can add only same-hash structural edges, so it partitions each incomplete family by exact hash before judging. A hash group with fewer than two members cannot add a pair and is skipped. Within a mixed family, every same-hash pair is independently certified so greedy grouping cannot hide a real copy when the relation is nontransitive; a family with one hash retains its original grouping. Certified pairs still go through the existing candidate builder, preserving existing candidate edges and their evidence. This bounds expensive judgement by same-hash group sizes rather than every cross-hash combination. `pipeline/session/information.rs` implements the partition and reports the number of potentially judged same-hash pairs; `recovery_judges_only_equal_hash_members_of_a_mixed_shape_family` and `hash_partition_preserves_old_recoverable_edges_with_nontransitive_kinds` pin both requirements.
...
`fused` is the pair's measured shape/semantic score `max(S,J,E)` ([FUSED-STRATEGY-BOUNDED-MAX]). It is distinct from the initial admission score in [FUSED-PRE-RESCUE-SCORE]. Content support `max(A,R)` stays separate. A cluster contains none of these measurements: it carries its kind, membership, and the mass and rank defined in [RANK-MASS-SUM](pipeline.md#rank-mass-sum-rank-by-duplicated-mass-only). Pair evidence is available only when comparing those two occurrences.

#### [FUSED-THRESHOLD] The pair admission bar

`admission.fused_threshold` (default 0.85) is the ordinary pair admission bar; provenance in [FUSED-TUNING-LEVERS]. A cross-language candidate without exact Merkle equality uses `admission.cross_language_fused_threshold` instead. Both apply to the initial score in [FUSED-PRE-RESCUE-SCORE]. Threshold configuration lives in [exclusion.md](exclusion.md); the remaining migration is tracked in `unhardcode-tuning-plan.md`.

### [FUSED-SHARED-SUBTREE] `structural` is measured subtree overlap, not Merkle equality

`structural` is the **best-achievable ordered subtree overlap** between two occurrences: `1 - TED / max(nodes)`, where `TED` is the Zhang–Shasha tree edit distance over normalised node kinds with unit insert/delete/relabel costs (`overlap.rs`). Merkle-equal occurrences short-circuit to `1.0`, so every exact pair keeps that value; what changes is the other end, which used to be a literal `0.0`. The alignment is measured for two populations and no other: the rescue candidates — pairs the fused floor, or the stricter LSH-only token floor, would drop despite token corroboration — and token-carried pairs, whose content floor under [FUSED-CONTENT-GATE] turns on whether the alignment clears the overlap floor.
...
A token-only pair that clears the fused floor (0.85) but not the LSH-only floor (0.90) is refused at construction, so it is a rescue candidate too. Otherwise a pair with weaker token evidence below the fused floor is measured while a stronger one is dropped unmeasured. The whole-namespace pair of `csharp-type3` sits in that band; `csharp_type3_reports_the_enclosing_method_pair` and `lsh_only_pairs_below_their_jaccard_floor_reenter_through_the_rescue_route` pin it.
...
**Rescue requires shared copied code.** Check overlap, tokens, size, the aligned core's content and the scope rules below. This proof can satisfy the ordinary or token-only admission guard. It applies across files and to the qualifying declarations in [FUSED-SHARED-SUBTREE-SAME-FILE]. Discovery and classification must reuse the same scope and copied-code checks in `overlap/rescue.rs::RescueContext`, including [FUSED-SHARED-SUBTREE-ECHO].

**Category labels and their separate thresholds** come from [CLONE-BUCKETS-ROUTING](taxonomy.md#clone-buckets-routing-classification-must-match-the-definitions).
...
So a rescued pair is judged on its **aligned core**: an order- and nesting-preserving pairing of the two endpoints' subtrees, built the way an ordered tree alignment maps nodes. Among two sibling sequences, Merkle-equal subtrees are paired by the heaviest order-preserving common subsequence, weighted by node mass so a twenty-node loop is never traded for two three-node declarations; the siblings left over on both sides are paired by kind, again in order, and the pairing descends into their children down to the leaves. Two stretches that share no kind may still be one code inside a shell — a whole file against the namespace it declares, a class against the body of the class beside it — and an alignment deletes the shell and maps what it wrapped, so a lone container on either side is unwrapped and its children aligned against the other side. That is how the namespace and class shells around `csharp-type3`'s methods reach the methods' core and publish at the whole-file extent the shells make genuine ([PIPELINE-CLUSTER-EXACT-SCOPE]). A statement moved across a loop is paired with itself and the loop stays in the core — a greedy cursor walk, sound as a mass bound, claimed the moved statement early, jumped past the whole `for` of the reordered `ledger` pair, then paired a nine-byte `entry * 3` with an unrelated `… * 2` and read a verbatim reorder as 0.6 agreement. Each pair of spans is one normalised shape, so their frontiers align position for position, and the concatenated core is measured with the gate's own axes at the gate's own floor, `content_gate.support_floor`, or admitted as a contradiction-free rename ([FUSED-CONTENT-GATE-RENAME]). A Type-3 near-miss is a Type-1 or Type-2 clone with an edit; the core is that clone, and if the core is not a copy the wider view cannot be one. `csharp-type3`'s core is its `if`, its declaration, its loop header, its body statement and its `return` — every name renamed and corroborated, every literal preserved — and it clears the floor. The welded tail's core is the two `boundingBox` lines, two one-shot substitutions beside two drifted selectors, and it does not. An empty core, or one that cannot be resolved, is refused: nothing measured vouches for nothing.

Each indexed fingerprint belongs to its exact normalised node. Byte ranges are not unique node identities: a Python loop body with one statement shares its range with that statement and its expression. Adding a second statement to the other copy must not replace the first statement's hash with its wrapper's hash, lose its credited nodes, or pair its leaves with the inserted statement. `fingerprint::visit_fingerprints` retains node identity through the existing hash walk; `overlap/core.rs` uses that identity for lookup. `overlap::core::tests::wrapped_spans::the_core_pairs_the_statement_a_single_statement_body_wraps` pins both the single-statement body and the wider control.

**The core must itself be a clone the scan would report.** Before it is measured, the core's paired spans must carry at least `min_nodes` nodes — the scan's own floor, below which it reports no subtree at all — and a core below it admits nothing. The floor is the scan's rather than the rescue's endpoint floor because a near-miss is by construction smaller than its endpoints: `python-type3`'s methods clear the endpoint floor with cores of 22 to 25 nodes, and at `--min-nodes 8` those are clones the scan reports (`type3_enclosing_method`). Three `site/tests` scenarios open with the two Playwright lines every browser test starts with, a viewport and a `goto`, and two of them go on to copy a third statement; the third shares that preamble and nothing else, yet its core — the two lines and one identifier, twenty nodes — measured exactly the content floor and rode the rescue onto the copy, one row wider than the members it joined. Two lines of framework preamble are not duplication the scan reports on their own, and a pair cannot be admitted on what the scan would never report (`a_shared_preamble_never_rescues_a_stranger_onto_a_copied_run`).

Three things about the core follow from what it is. A changed **operator** is read over the whole endpoints before the core is measured, because the core holds only what the two share and a changed computation is exactly what they do not: `credit`'s `+` against `debit`'s `-` lives outside every Merkle-equal span, and the operator-drift pair may not become a near-miss by having its difference cut out of the evidence (`operator_drift_is_not_duplication`). A changed **call target** is read over the core itself: telling a renamed collaborator's method from a changed operation needs positions ([FUSED-CONTENT-GATE-CALL-TARGET]), near-miss endpoints have none, and the core lines up by construction (`type2_rename_call_targets`). And the rename test's **vocabulary clause** — that a copy keeps at least as many names as it renames — is not asked of the core: it separates a Type-2 clone from a different vocabulary over one shape, which is a question the rescue's ordered-overlap and token floors have already answered for this pair, and a near-miss method that renames every local keeps nothing by that count. `python-type3`'s core renames `bound`, `running` and `step` throughout and keeps only `range`; asked to keep as many as it renamed, the canonical near-miss would be refused (`type3_enclosing_method`). Every other clause holds: no substituted position may contradict the bijection, and at least one substitution must be corroborated — which is what still refuses the welded tail, whose two substitutions are each seen once.

When two emitted single-subtree fingerprints have the same Merkle hash, that whole subtree is their aligned core. The core can be returned without indexing every descendant. A file wrapper that only repeats its child is deliberately not emitted and still aligns through its child. `equal_single_subtrees_are_the_whole_core_without_descendant_indexing` and `viewless_wrapper_keeps_its_child_as_the_core` pin both cases.

This makes admission monotone in extent — no pair can be admitted by widening a pair the gate refused — and it retires the whole-endpoint floor that sat beneath a whole-endpoint gate. Implemented in `overlap/core.rs` (`OverlapMeasurer::aligned_core`, and `judge_core` — the one verdict both readers share) and `content::ContentPair::core`; applied in `overlap/rescue.rs` and, for an explicit comparison, in `pipeline/session/pair_compare`; pinned by `cluster_extent_statement_runs::a_scenario_tail_is_never_welded_onto_a_copied_scenario` and `a_shared_preamble_never_rescues_a_stranger_onto_a_copied_run`, `type3_enclosing_method` across every language, `lsh_only_nearmiss_recall` (the reordered pair), `rank_mass` (the `verbatim-plus-stranger` stranger), and `overlap::tests::the_aligned_core_pairs_the_shared_subtrees_in_order` and `the_aligned_core_keeps_the_loop_a_moved_statement_crossed`.

### [FUSED-SHARED-SUBTREE-CORE-NEED] Measure the aligned core only when its verdict can matter

An explicit pair or cluster-kind comparison needs an aligned-core verdict only while one of three outcomes can still change: the pair meets the rescue's scope, overlap, token, and minimum-size floors; a rejected near-identical shape could become an informational structural match depending on whether its core is copied; or the existing admission rules accept a pair that has no category yet from text identity, whole-content support, or embedding evidence. For a pair that cannot be rescued, the same admission rules are evaluated with an absent core before deciding whether to measure it. When none of the three outcomes can change, the core is skipped and its verdict is treated as absent. `core_need::tests` pins both the skip and every reason to retain the measurement; pair-comparison and corpus report assertions pin the rendered outcomes.
...
For a pair `p`, `structural` is its measured shared-subtree overlap ([FUSED-SHARED-SUBTREE]), `token_jaccard` is the MinHash Jaccard estimate between its two signatures, and `embedding_cos` is the cosine of its two vectors under [FUSED-EMBED-PROVIDER]. Missing input is `0.0` on that pair's axis. The same pair owns `agreement`, `rename_consistency`, and literal fraction under [FUSED-CONTENT-GATE], and `text_identity`: whether its two raw ranges are the same bytes (`byte_identical`), the same lines once each line's leading whitespace is removed (`indentation_only`, so a reader can confirm at a glance that a diff of them shows indentation and nothing else; line endings and a missing final line break are not compared), or `different`. Text identity never changes admission or classification.

An explicit pair comparison identifies both endpoints and may render that pair's `S`, `J`, `E`, `A`, `R`, literal fraction, and admission explanation in one compact secondary row. Cluster JSON, text, HTML, LSP, MCP, editor, and AI-context surfaces render none of those values. A cluster contains its occurrence membership, duplicated mass, and clone kind; closure does not inherit evidence from any edge, and the kind is established before clone-only counting and ranking ([CLONE-KIND-FOLD]). Rejected comparisons are not a fallback clone category; preserve genuine clone subsets separately from informational matches.
...
An endpoint's raw-content frontier may be needed by several pair comparisons and by both whole-endpoint and aligned-core checks. Resolve it once per exact fingerprint within a comparison session, rescue worker, or pre-closure content-gate pass. Identity includes the file, byte range, Merkle hash, and node count: a syntax wrapper and its child can share a byte range but have different content frontiers. The bounded cache rotates when full so later endpoints can also be reused. Caching must preserve every whole-content and core evidence value; `rescue_core_reuses_resolved_content_frontiers` pins worker reuse against the uncached parity controls, and `gate_reuses_frontiers_across_exact_content_edges` pins pass-wide gate reuse. A cheap resolved-endpoints check may replace calculating the whole evidence solely to read its `measured` bit; `resolved_matches_whole_evidence_for_present_and_missing_endpoints` pins the equivalence for both present and invalid ranges.

#### [FUSED-CONTENT-GATE-EXACT-FRONTIER] Matching authored positions need no rename calculation to pass

Two resolved endpoints with the same normalised shape and the same **ordered collapsed-leaf content keys** have whole-content agreement `1.0`. When the ordinary operator and call-target check also finds no contradiction, the pre-closure content gate may admit the pair directly if `1.0` clears that pair's configured floor. This skips rename and literal calculations that cannot change the gate verdict. Matching source bytes or Merkle hashes alone are insufficient: the same bytes can be divided into different collapsed leaves, so the key sequences differ and the pair must be measured normally. Unresolved pairs, changed operations, and pairs below their content floor also keep the canonical measured route. `exact_frontier_route_matches_canonical_clearance`, `changed_operator_stays_measured_and_refused`, and `equal_hash_and_bytes_can_have_disagreeing_content_frontiers` pin the boundary; pair comparison still calculates complete content evidence for the report.

#### [FUSED-CONTENT-GATE-CALL-TARGET] Changing an external operation is not a local rename

Calls such as `toHaveCount` and `toContainText` perform different operations. A method declared outside the copied region is a fixed name the copy did not author, so changing one contradicts the copy; a method declared inside the region may be renamed with its callers. But a systematic rename is still a copy ([CLONE-BUCKETS-NORTH-STAR](taxonomy.md#clone-buckets-north-star-what-the-four-categories-mean)), and Deslop tells a rename from a changed operation by the receiver — the name the call is made on, whether a plain handle (`ledger` in `ledger.postEntry(..)`) or a property (`b` in `a.b.c()`). A changed selector is excused when that receiver is itself a corroborated rename: the same name mapped the same way more than once, inside a mapping nothing contradicts ([TECH-PMATCH-BAKER]), at the same frontier position in both occurrences. `ledger.postEntry` against `journal.writeRecord`, in a function whose receiver, parameters, locals and types all rename together, is one function written twice; `client.startJob` against `client.cancelJob` renames nothing else and asks one collaborator for two different things, however many times the swap repeats. The selector's own repetition is not required — `self.order_repo.fetch` against `self.user_store.load` renames each method once on a collaborator renamed throughout (`python_inherited_contract_boundary`) — and a call made on something that is not a name, `expect(page.locator(..)).toHaveText(..)`, has no receiver to corroborate and is never excused. The rule reads positions, so it applies only where the two occurrences line up: across a same-shape pair, and across the shared code of a near-miss pair ([FUSED-SHARED-SUBTREE-CORE]). Implemented in `content/call_targets.rs`; pinned by `type2_rename_call_targets`, `js_literal_variation_calls::member_targets` and `cluster_extent_statement_runs`.

An external selector gaining or losing exactly the `Async` suffix is a bounded edit, not a receiver rename and not an unrelated-operation veto. It may rescue a sync/async twin only when AST-sourced selector bytes prove that edit, the aligned core independently passes the ordinary content gate, structural overlap and token similarity clear their existing floors, and the whole endpoints contain no hard contradiction. An added async argument can lower whole-endpoint key-set agreement without erasing the copied core. The complete C# pair, its Type-3 bucket, extent, and rank are pinned by `csharp_sync_async_call_targets`; changed operations on the same receiver remain rejected by `type2_rename_call_targets` ([ACCURACY-RECOVERY-TWIN-FN]).

Keep the actual content measurements even when a contradiction rejects the pair. Rejection does not mean zero similarity. Truly unavailable measurements are unknown under [REPORT-CONTEXT-PAIR](REPORTING-CONTEXT.md#report-context-pair-comparing-two-occurrences).
...
#### [FUSED-CONTENT-GATE-RENAME] A consistent rename is admitted whatever its literals do

The pooled rename axis prices a rename by its anchors, and a rename can be complete yet anchor-poor. Sibling accessors whose literal keys rename with the accessor (`'Name'` beside `getName`, `'Value'` beside `getValue`) preserve no literal, so the axis reads `0.0`. An `if` chain mapping the same constants to a renamed family of return constants substitutes each name once, so nothing corroborates it and the axis reads `0.6`. Two consistently renamed test groups read `0.34`. All three measured under `content_gate.support_floor`, were dropped before closure, and are one code written twice — the Type-2 definition, [TECH-PMATCH-BAKER]'s parameterised match itself.

So the measured route admits a pair on either of two grounds: its pooled support clears the floor, **or** the pair is a rename — its frontiers align, no substituted identifier position contradicts the modal bijection, at least one substitution is corroborated, and the copy keeps at least as many names as it renames (the one clause the aligned core of a rescued pair is excused from — [FUSED-SHARED-SUBTREE-CORE]). A substitution is corroborated by the same symbol renamed the same way twice, by a literal echoing it, or by sibling symbols renamed by one transformation — `STREAM_CRYPTO_PROTO_TLSv1_0 -> CURL_SSLVERSION_TLSv1_0` and its `_1` and `_2` siblings are one rename of a prefix seen three times. A literal that drifted with the rename is not evidence *against* the copy, so it may not refuse it. Three things still do: a family whose every substitution is seen once and shares no transformation, which proves no rename at all; an inconsistent substitution — one name mapped two ways — which is what keeps a minified polyfill function from being admitted against `axios`'s `createInstance` on shape alone (`agreement = 0.0`, an inconsistent mapping); and a family that renames more names than it keeps, which is a different vocabulary over one shape rather than a copy — the `ts-interfaces` data shapes keep five names and rename thirteen, the `csharp-issue-134` handlers keep one and rename thirteen, while the `SetCookie` accessors keep four and rename four, the TLS chain keeps six and renames three, and the `bloc` groups keep nine and rename three. A pair whose identifiers all agree is not a rename at all and is judged on its agreement as before. The pooled axis is unchanged and still routes; only the refusal it alone could issue against a rename is gone. Implemented in `content/rename.rs` (`pair_rename_is_consistent`) and read by the gate in `pair/content_gate.rs`; pinned by `crates/deslop/tests/type2_rename_literal_drift.rs` and, for the refusal that stays, by the `axios` judged-out pair of the corpus scorecard.

#### [REPAIR-RENAME-LITERAL-ECHO] A literal renamed with the symbol it names is part of the rename

A literal that changed *alongside the symbol it names* is the rename carried out thoroughly, not evidence against it. `"OrderService"` becoming `"UserService"` beside the `OrderService` symbol is the same edit reaching the string that spells the name, and counting it as a differing literal inverted the score outright — a half-finished rename, which left the string alone, outscored the complete one.

An **echo** is recognised by content and never by coincidence. The literal's bytes must transform into the partner's bytes by exactly the substitution the identifier bijection already explains; anything else is an ordinary differing literal and still refuses. A literal that qualifies then corroborates that substitution the same way a second occurrence of the renamed identifier would, so a rename spelled once in code and once in a string is anchored, not unsupported.

Implemented in `content/rename/literal_echo.rs` and read through `content/rename.rs`. Pinned by `crates/deslop/tests/rename_literal_monotonicity.rs`, which holds the direction that broke: finishing a rename may never lower the score.
...
Weight, the unchanged AST-node mass formula, exclusions and ordering are defined once in [RANK-MASS-SUM](pipeline.md#rank-mass-sum-rank-by-duplicated-mass-only).

*Pair evidence decides pair admission. Closure forms the cluster. Mass alone weighs it.*

**Repo metrics** — [METRICS-REPO](pipeline.md#metrics-repo-repo-wide-duplication-metrics)
...
Every numeric cutoff for detection, admission, classification or suppression must have a named default and a `.deslop.toml` setting. Validate it at load and record its effective value in the report ([EXCLUSION-CONFIG], [CONFIG-TUNING-DECLARED]). Existing defaults are listed here; new classification defaults are listed separately in [CLONE-BUCKETS-THRESHOLDS](taxonomy.md#clone-buckets-thresholds-defaults-and-toml-settings) and require corpus validation. Rank actual clones using the unchanged mass formula.
...
| `content_gate.promote_floor` | `buckets.rs` | 0.85 | **Derived**. The floor an unanchored LSH-only pair pays in every scope; bounded below by the #339 whole-file-against-interior-window pairs, which ride that route at cross-file support strength. It is not a within-file floor: the single-file REST settings family (0.72–0.80) is convicted by [RANK-STRUCTURAL-ONLY-FORWARDING] after closure, and a same-file copy varying only its literals (0.73) is a finding. |
| `content_gate.structural_only_max_support` | `buckets.rs` | 0.05 | Noise ceiling for token/embedding signals. It is not a measurement of content similarity and must not define shape-only or override a content rejection ([CLONE-BUCKETS-STRUCTURAL-ONLY](taxonomy.md#clone-buckets-structural-only-shape-only-is-not-duplication)). |
