...
This makes admission monotone in extent — no pair can be admitted by widening a pair the gate refused — and it retires the whole-endpoint floor that sat beneath a whole-endpoint gate. Implemented in `overlap/core.rs`, `OverlapMeasurer::aligned_core` and `content::measure_aligned_core`; applied in `overlap/rescue.rs` and, for an explicit comparison, in `pipeline/session/pair_compare`; pinned by `cluster_extent_statement_runs::a_scenario_tail_is_never_welded_onto_a_copied_scenario`, `type3_enclosing_method` across every language, `lsh_only_nearmiss_recall` (the reordered pair), `rank_mass` (the `verbatim-plus-stranger` stranger), and `overlap::tests::the_aligned_core_pairs_the_shared_subtrees_in_order` and `the_aligned_core_keeps_the_loop_a_moved_statement_crossed`.
