...
4. **Admission is decided pair by pair.** A pair must pass the size-coherence and applicable LSH-only guards, clear its pair-specific shape/semantic threshold or the shared-subtree rescue, and pass every applicable pair-content guard. Group-level similarity judgement and averaging are forbidden. Clusters are the transitive closure of admitted pairs before the separate convicted-noise rule in [CLONE-NOISE-VERBATIM-SUBGROUP].
...
### [FUSED-SHARED-SUBTREE-SAME-FILE] A disjoint near-miss inside one file uses the rescue evidence

Two methods that drifted apart inside one file are the same duplication as two that drifted apart across files: the file boundary records where the copy was pasted, not whether it is a copy. A same-file pair is rescue-eligible on the terms of [FUSED-SHARED-SUBTREE] — the overlap, token Jaccard, endpoint node-count, size-ratio and raw-content-agreement floors — with one condition of its own: its endpoints are disjoint. Nested or overlapping same-file endpoints are one region read twice and are refused before measurement. The corroboration is the rescue's own raw-content-agreement floor, `rescue.content_agreement_floor`, in both scopes: the content gate's `promote_floor` (0.85) is a routing floor for saturated shape and token evidence, and a Type-3 insertion is exactly what keeps such a pair from saturating — `csharp-merge-drift`'s two methods measure overlap 0.82 with `max(agreement, rename_consistency)` 0.56, so holding the rescue to 0.85 would refuse the one pair the route exists to admit. Refusing the route outright, as 0.32.0 and the first 0.33.0 tip did, published only the exact statement fragments those two methods share and never the methods.

[FUSED-SHARED-SUBTREE-ECHO] applies unchanged: the exact-function index holds same-file clones too, so two sibling classes in one file that each wrap a byte-identical method are refused — their shared mass beyond the method is below the node floor — and the method publishes at its own lines in both classes (`same_file_rescue::sibling_classes_wrapping_one_exact_method_publish_the_method`).

Implemented in `pair/candidates/builder.rs` (admission), `overlap/rescue.rs` (disjoint same-file measurement) and `pair/echo.rs` (same-file anchors); pinned by `type3_enclosing_method::csharp_same_file_type3_reports_both_methods_in_one_cluster` and `same_file_rescue`.
...
A pair is admitted when its pre-rescue score clears its threshold or the shared-subtree rescue fires, subject to the size-coherence and LSH-only guards. Rescue requires its own raw-content agreement and either cross-file endpoints or disjoint same-file endpoints; those conditions are load-bearing parts of the implementation, not optional prose.

$$
\begin{aligned}
\mathrm{rescue}(p) \iff {}& \bigl(\operatorname{cross\_file}(p)
\lor (\operatorname{same\_file}(p) \land \operatorname{disjoint}(p))\bigr) \\
