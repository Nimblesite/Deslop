...
- [x] Ollama HTTP client (`embedding/ollama.rs`) — loopback-only, no TLS dep; `/api/tags` → digest; batched `/api/embed` → vectors. Default model `nomic-embed-text` (137 M params, 768-dim, Apache 2.0). Rationale: ensemble-LLM 2025 finding that "smaller embedding sizes, smaller tokenizer vocabularies and tailored datasets are advantageous"; user-overridable via `--embedding-model` (swap to `nomic-embed-code` / `codet5p` / `unixcoder` once pulled locally).
...
- [x] Cluster detail action and hover hardening ([VSIX-WEBVIEW-ACTIONS-CONTEXT]) — wire footer cluster navigation to the same selected-cluster signal as `n/p`, remove host-side no-op navigation handling, and add detailed hover context for visible data and action controls. Tests prove every rendered control is backed by a real behavior and that hover text explains rank, bucket, signals, occurrences, hidden/canonical status, Open, Compare, and keyboard help.
