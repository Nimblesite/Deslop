...
- [x] `RepoMetrics { analysed_loc, duplicated_loc, duplication_percent, clusters_total, duplicated_files, threshold }` in `codededup-core::report_metrics`. Computed by projecting every non-hidden `ReportOccurrence` onto a per-file `BTreeSet<line>`, unioning, and summing set sizes — overlapping sibling-extension ranges count once.
...
- [x] `[threshold] max_duplication_percent` in `.codededup.toml`, parsed in `codededup-core::config::ExclusionConfig`. CLI flag beats config; `--no-fail-over` beats both.
...
- [x] `live` cargo feature on `codededup-core` that pulls in `notify` + `tokio` + `futures` + `async-trait` as workspace-pinned optional dependencies. CLI build path stays feature-off; `codededup-lsp` opts in. Whole module guarded by `#[cfg(feature = "live")]`.
- [x] `codededup-core::live::AnalysisSession` at `live/session.rs` owns one `PipelineSession`, an `Arc<Report>` snapshot, monotonic `generation: u64`, and active embedding provider ([LIVE-STATE]).
- [x] `codededup-core::live::LiveApi` trait + `LiveService` impl at `live/api.rs` exposing the nine query methods ([LIVE-QUERY-API]).
...
- [x] E2E harness at `crates/codededup-core/tests/live.rs` (feature-gated) — 8 tests: first-report-matches-batch, update_files delta, find_similar open-range / snippet / unparseable / below-min-nodes, debouncer cap, stub-only fallback, and a `LiveService` round-trip covering every `LiveApi` method.
...
- [ ] Deferred (post-P8, tracked for later): code lens ([LSP-CODE-LENS]), hover ([LSP-HOVER]), `definitionProvider` overload, `codededup://` virtual document scheme ([LSP-VIRTUAL-DOC]), `workspace/executeCommand` verbs ([LSP-COMMANDS]) — the LSP shell currently ships the request-response surface; these UX surfaces land with the VSIX work in P10.
...
- [x] New crate `crates/codededup-mcp` — `McpBackend` trait + `PipelineSessionBackend` concrete impl forwards to `codededup_core::PipelineSession` (swapping to `LiveApi` once the live-feature trait stabilises is a one-line swap inside `backend.rs`). Six files: `lib.rs` / `protocol.rs` / `safety.rs` / `backend.rs` / `tools.rs` / `resources.rs` / `server.rs` / `main.rs`, each < 500 LOC.
...
- [x] Resources: `codededup://report` (canonical JSON, pretty-printed) and `codededup://schema` (markdown) via `resources/list` + `resources/read` ([MCP-RESOURCES]).
- [x] `notifications/codededup/filesChanged` incoming notification calls `PipelineSession::update_files` and bumps the generation counter so agents can push watcher edits without a tool call round-trip. Outgoing notifications follow once the watcher layer (P7) lands the push pipeline.
...
- [x] E2E: `crates/codededup-mcp/tests/cli.rs` drives the real binary over stdio with raw JSON-RPC frames — 50 tests covering the initialize handshake, `tools/list` shape, every tool (happy + error), resources list + read, path-traversal rejection, malformed-frame parse error, unknown-method error, invalid jsonrpc version, notifications/initialized + files-changed, string-id round-trip, empty-line tolerance, Ollama-auto fallback to stub, and binary CLI arg validation.
- [x] `make ci` green: fmt check + clippy pedantic+nursery + 50 MCP e2e + existing 72 CLI + 7 rerun + 7 live + 5 LSP tests. Coverage 96.1% (above 95% threshold + 1% rounding slack). `codededup-mcp/src/main.rs` + `server.rs` excluded from coverage (transport shells exercised only via subprocess E2E; llvm-cov can't see instrumented child output). Ollama-backed variants sit behind the `ollama_` prefix and run under `make ci-ollama`.

### P10 VSIX + live bubble — v0.1 LANDED (codededup-opus-main)
...
- [x] Binary resolver ([VSIX-BINARY-VERSIONING]) — `src/binary.ts`: `${CODEDEDUP_BINARY_DIR}` → `PATH` (accepted only when `--version` matches the VSIX version exactly) → bundled `${extensionPath}/bin/<platform>/`; on bundled, prepends that directory to the VS Code session `PATH`. `codededup.revealActiveBinary` surfaces the resolved path.
...
- [x] Cluster detail webview (`codededup.openCluster`) — Preact component at `webview-ui/src/cluster/main.tsx`: header with severity badge + rank + weight + canonical path, 4-bar signal strip, alternating-row occurrence list, `j/k/n/p/Enter/?` hotkeys, signals-driven re-render on every delta.
- [x] Full report webview (`codededup.openReport`) — `webview-ui/src/report/main.tsx`: display-large duplication percent, cache + state footer, filter row (language / severity / path glob), worst-first list, signal-driven refresh on every report delta.
- [x] Marketplace + OpenVSX publishing pipeline in [.github/workflows/publish-vsix.yml](../../.github/workflows/publish-vsix.yml) — Rust matrix builds (`darwin-arm64`, `darwin-x64`, `linux-x64`, `linux-arm64`, `win32-x64`), `version-check` job fails when `clients/vscode/package.json` and the Cargo version disagree, `vsce package` + `vsce publish` + `ovsx publish`.
