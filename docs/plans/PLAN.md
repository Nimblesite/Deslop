...
- [x] `[threshold] max_duplication_percent` in `.codededup.toml`, parsed in `deslop-core::config::ExclusionConfig`. CLI flag beats config; `--no-fail-over` beats both.
...
- [x] Binary resolver ([VSIX-BINARY-VERSIONING]) — `src/binary.ts`: `${CODEDEDUP_BINARY_DIR}` → `PATH` (accepted only when `--version` matches the VSIX version exactly) → bundled `${extensionPath}/bin/<platform>/`; on bundled, prepends that directory to the VS Code session `PATH`. `deslop.revealActiveBinary` surfaces the resolved path.
...
- [x] Bundle per-platform pre-built `deslop-lsp` + `deslop-mcp` binaries at release time — `build-binaries` matrix in [publish-vsix.yml](../../.github/workflows/publish-vsix.yml) emits artifacts that `package-vsix` flattens into `clients/vscode/bin/<platform>/` before `vsce package`. No download-on-activate.
