...
- [ ] `cargo tree -p deslop-core | grep tree-sitter` — snapshot the
      current transitive tree before touching anything.
- [ ] `ls .devcontainer/` — inventory devcontainer files that might
      reference tree-sitter versions.
- [ ] `grep -rn "tree_sitter::" crates/ --include='*.rs'` — confirm
      the touchpoint list matches [TS-UPGRADE-TOUCHPOINTS]. Any missed
      callsite is a Phase 0 deliverable.
- [ ] Confirm `tree_sitter::LanguageError` path survives 0.26 (quick
      `cargo doc --open` on a scratch crate or scan docs.rs).
- [ ] Stash the current `make ci` output as the baseline (clone cluster
...
- [ ] Create `crates/deslop/tests/fixtures/ast-golden-rust/` with
...
- [ ] Create `crates/deslop/tests/fixtures/ast-golden-python/` with
...
- [ ] Add `debug_ast_dump_matches_committed_golden` tests mirroring
      the C# one for both languages.
- [ ] `make test` — confirm all three goldens pass on the current
      0.22 runtime. This is the pre-upgrade snapshot.
...
- [ ] Edit `Cargo.toml`:
      ...
      ```
- [ ] `cargo update -p tree-sitter -p tree-sitter-c-sharp -p tree-sitter-rust -p tree-sitter-python`.
- [ ] `cargo build --workspace` — **expected to fail** with
...
- [ ] Run the three golden tests. They will fail on grammar bumps.
      ...
      - **Reject** renames that would stop collapsing
        ...
        rubber-stamping the golden).
- [ ] For every intentional golden change, update
      `Sample.expected.ast`, commit with a message explaining the
      grammar version delta.
- [ ] For every `normalise_kind` that had to grow a new arm (new
...
- [ ] `make test` — fail-fast, coverage ≥ `coverage-thresholds.json`.
      **Coverage must not drop.** If it does, the normalisation
      regressed and Phase 4 accepted a bad diff.
- [ ] `make build` (release).
...
- [ ] Run the CLI against
      ...
      this doc under [TS-UPGRADE-POST-MORTEM].

### Phase 6 — CI pin-drift check

- [ ] Verify [.github/workflows/ci.yml:29](../../.github/workflows/ci.yml)
      loop still passes (regex already tolerant). No edit expected.
- [ ] **Optional** add `tree-sitter-language` to the pin-drift loop.
...
- [ ] Audit `.devcontainer/` for any `tree-sitter` version strings
      (Dockerfile, `devcontainer.json`, `setup.sh`). Update to match
      `Cargo.toml`.
- [ ] Rebuild the devcontainer locally if available; confirm
      `make ci` passes inside it.

### Phase 8 — Documentation sync

- [ ] [docs/plans/PLAN.md](PLAN.md) — add P-LANG-0 entry linking
      here.
- [ ] [docs/plans/LANG-ROADMAP.md §LANG-ROADMAP-RUNTIME-UPGRADE](LANG-ROADMAP.md) —
      mark runtime upgrade complete, update version-grid rows.
- [ ] [docs/specs/pipeline.md §PIPELINE-LANG-TRAIT](../specs/pipeline.md) —
      if the paragraph names specific grammar versions inline,
      refresh them.
- [ ] [CLAUDE.md](../../CLAUDE.md) — **read-only in this PR**; no
...
Populate after Phase 5. Expected sections:

- **Grammar-release delta per language.** Summary of what each grammar
  actually changed between the old pin and the new pin (read the
  `CHANGELOG.md` in each grammar repo).
- **`normalise_kind` edits.** List of new arms added in Phase 4, each
  with a one-line reason.
- **Cluster-count delta vs. baseline.** Per fixture, before / after.
- **Coverage delta.** Before / after, plus any files whose coverage
  dropped most and why.
