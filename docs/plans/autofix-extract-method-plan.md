...
- [x] `[AUTOFIX-EXTRACT-PRECONDITIONS]` **rule 7 — write-in-span gate** (issue #280): `compute_plan` refuses extracts whose free variables are write targets inside the span — merge check D's written-hole dataflow applied to the extract's context parameters (silent mutation loss defeats the type-safety backstop). `write_kinds` moved from `MergeTables` to `ScopeKinds` as structured `WriteKind` patterns covering assignments (Python declares `augmented_assignment` only — plain assignment binds and is rule 6's territory), marker-token writes with no target field (C# `++`/`--` and `ref`/`out` arguments, Dart increments), and destructuring targets (C# tuple deconstruction, Dart pattern assignment); spans declaring Python `nonlocal` refuse outright (relocation breaks the binding; `global` survives and extracts). The merge tier runs the same check over its *context* free variables (`context_write_check`) — check D alone only covered holes. Shared machinery lives in `refactor::read_after` beside rule 6. E2E: `crates/deslop-core/tests/refactor_extract_write_gate.rs` (12 tests: refusals for compound/plain/write-only assignment, increment, `out` argument, tuple deconstruction, `nonlocal`; positives proving span-*bound* writes and `global` spans still extract) plus `written_context_variable_refuses` / `dart_written_context_variable_refuses` in `refactor_merge.rs` (fixtures `csharp-merge-writtencontext`, `dart-merge-writtencontext` — the latter is the only live coverage of Dart's write tables). The two rule 5 alignment tests retargeted to windows that write no free name (their old windows asserted the corrupting pre-#280 behaviour).
