...
> **Spec is the source of truth.** Behavioural shape, the mergeability gate, the anti-unification algorithm, preconditions, emitter rules, per-language defaults, destination policy, and caveats live in [autofix-extract.md](../specs/autofix-extract.md) under `[AUTOFIX-*]` (`[AUTOFIX-EXTRACT]`, `[AUTOFIX-MERGE]`, `[AUTOFIX-CONSOLIDATE]`, `[AUTOFIX-ZERO-RISK]`). This file describes only **how** to build it. Implementation order: Type-1 verbatim extract → leaf-gap merge → cross-file consolidation; each reuses the `refactor` module the prior tier establishes.
