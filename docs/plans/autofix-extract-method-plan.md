...
- [x] `[AUTOFIX-EXTRACT-PRECONDITIONS]` **rule 7 — write-in-span gate** (issue #280): `compute_plan` refuses extracts whose free variables are bare-identifier assignment targets inside the span — merge check D's written-hole dataflow applied to the extract's context parameters (silent mutation loss defeats the type-safety backstop). `write_kinds` moved from `MergeTables` to `ScopeKinds` so both tiers share one table (Python declares `augmented_assignment` only — plain assignment binds and is rule 6's territory); the shared `written_in_span` lives in `refactor::read_after` beside rule 6. E2E: `crates/deslop-core/tests/refactor_extract_write_gate.rs` — `csharp_written_free_variable_refused`, `python_augmented_assignment_of_free_name_refused`, `rust_compound_assignment_of_free_name_refused`, plus positives proving writes to span-*bound* names still extract; the two rule 5 alignment tests retargeted to windows that write no free name (their old windows asserted the corrupting pre-#280 behaviour).
