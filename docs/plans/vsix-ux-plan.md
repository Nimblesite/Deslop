...
## Cluster and explorer usability

The comparison and help changes follow [VSIX-WEBVIEW-ACTIONS-CONTEXT] and [VSIX-PAIR-COMPARE] in [webview-runtime.md](../specs/webview-runtime.md). Keep file navigation in the file link, show canonical and selected-pair actions separately, and place immediate Markdown help at section level. Browser assertions cover both viewport sizes and the exact comparison endpoints.

The explorer follows [VSIX-TOP-OFFENDERS-GROUPING], [VSIX-TOP-OFFENDERS-LANGUAGE-GROUP] and [VSIX-TOP-OFFENDERS-SORT] in [vsix.md](../specs/vsix.md): one grouping picker and fixed highest-weight-first ordering. Menu, grouping and extension command assertions cover the same contract.
