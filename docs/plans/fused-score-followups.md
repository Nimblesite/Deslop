...
## 3. Subsumption escapes — #389 and #421
...
## 4. Assertion instruments that pass vacuously
...
- [x] **#412** — substring skips replaced with a `required-features`-gated `corpus_repos` target; `make test` now runs the whole workspace unfiltered and `make lint` refuses a recipe that names a test. Every "suite is green" claim in this file still needs re-reading against a gate that actually runs those tests. The accidental-skip inventory is in item 4 above.
