...
- [x] Blob trust hardened per the regression audit ([`../incremental-persistence-regression-audit.md`](../incremental-persistence-regression-audit.md)): every blob carries a binding digest over `(magic, semantic epoch, signature width, min_nodes, language, **tool version**, source hash, payload)`, verified before a single payload byte is decoded; the read is bounded on the read itself (one handle, taken one byte past the 256 MiB ceiling, reserved fallibly) so a file grown mid-read is refused rather than truncated into a valid-looking prefix; a 4 M global decoded-node budget bounds the whole tree where `MAX_AST_DEPTH` bounds only one path ([PIPELINE-INCREMENTAL-INTEGRITY], pinned by `cache_blob_integrity.rs` + `fpcache/tests.rs`)
...
- [x] Retention landed ([PIPELINE-INCREMENTAL-RETENTION]): after every full store-on pass, **nothing is deleted under budget** — neither provable orphans (exactly the revert/branch-switch reuse set the equivalence suite asserts full-hits) nor another tool version's partition, which may belong to a second binary sharing the workspace that two mutually-sweeping binaries would deadlock into permanent rebuild churn. Over the 2 GiB budget, eviction is by class (other-version → orphan → live), then oldest-first, path as tie-break; class outranks age in both directions. Pinned by `cache_retention.rs` (other-version partition survives byte-identically with `other_version_blobs=1, evicted_blobs=0` logged, kept orphan full-hits the revert, disabled pass never sweeps) and `fpcache/retention/tests.rs` 7/7 (classification, class-before-age across all three classes, orphan-before-live, oldest-first fallback, budget stop, foreign-file safety, other-`min_nodes` liveness)
...
### Follow-ups
- [x] Pin persisted-signature recall on the LSH-only route — `lsh_only_nearmiss_recall.rs::the_lsh_only_pair_keeps_its_verdict_across_the_persistence_matrix`: cold, fully warm, mixed pass, and revert, each asserting `structural <= 0.01`, the exact token Jaccard, `nearly_identical`, `fused >= 0.85`, exact files/spread/metrics with a re-derived `duplication_percent`, byte-equal reports modulo `cache_stats`, and an **exact** mixed-pass `signatures_built` / `signatures_reused` split derived from a one-file measurement rather than hardcoded
- [x] `tool_version` bound into `BlobBinding` and the length-prefixed digest input alongside `SEMANTIC_EPOCH`; a blob relocated across version partitions is refused on both the decode path and the digest-distinctness axis test
- [x] Hostile-size gaps closed: one handle supplies length and bytes, the read is taken one byte past the ceiling so mid-read growth is refused rather than truncated, the buffer is reserved fallibly, and `MAX_DECODED_NODES` (4 M) bounds the whole tree — claimed per node including the child slots that follow it, so an absurd child count is refused before its `Vec` is reserved
- [x] Retention made safe across concurrently running tool versions: another version's partition is classified `OtherVersion` and never deleted under budget, only evicted first *under* pressure. Pinned at unit level (classification + class-before-age) and E2E (partition survives a sweep byte-identically)
