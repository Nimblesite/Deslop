...
### Three further ingestion defects — two quarantined, one repaired (found 2026-08-18)

An adversarial review of the shipped diff-scope code found five candidate defects; two independent verification lenses per candidate confirmed three and **refuted** one, which matters as much as the confirmations.

**Quarantined — C-quoted paths are never unquoted ([PIPELINE-DIFF-INGEST], silent false negative).** `new_side_path` returned the target verbatim, so `git`'s quoted form — `"b/caf\303\251.rs"`, quotes and octal escapes included — became the path. It matched nothing in the corpus, and an unmatched path is *ignored* rather than refused, so every clone added in that file went untagged: dropped by `--only-changed`, missing from the `added_loc` denominator, exit 0. `git` quotes any path with non-ASCII bytes, a quote, or a backslash with stock config, so one accented or CJK filename triggers it on the documented `git diff | deslop --diff -` flow. The spec lists C-quoted paths as recognised grammar. Red: `c_quoted_new_side_path_is_unquoted`.

**Quarantined — plain multi-file diffs merge into one file section ([PIPELINE-DIFF-INGEST], silent false negative).** Only a `diff ` line opened a section and `--- ` is swallowed as metadata, so a second `+++` overwrote the path while the first file's hunks stayed attached to it. Where the two files share the hunk's content — the copy-paste this tool exists to find — verification passes and the first file silently receives no added spans at all; where they differ it raises a `DiffStale` naming the wrong file. `git` and `hg` are unaffected; the triggers are `black --diff`, `ruff format --diff`, and `diff -u` loops. Red: `plain_multi_file_diff_keeps_each_file_section_separate`.

Both quarantines sit on the `--diff` path only — live, LSP and MCP sessions never pass a diff — so the editor surfaces are untouched, and the seven diff E2E tests stay green because a `git` diff never reaches either branch.
...
**Open, needs a deliberate decision.** `pipeline.md` says `clusters_total` "always equals `clusters.len()`", but `compute_repo_metrics` counts only clusters with at least two visible occurrences. On a mixed cluster the banner therefore says "0 clusters" above a body listing one. Code and `metric_excludes_hidden_clusters.rs` agree with each other and the spec sentence is the outlier — but which side moves is a product call, not a refactor.
