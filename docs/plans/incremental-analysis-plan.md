...
| [CLI-ARG-DIFF] + [CLI-ARG-ONLY-CHANGED] | The two flags and their conflicts | ⚠️ implemented and pinned by `diff_scoped_reporting.rs` 7/7; gated on the [PIPELINE-DIFF-INGEST] P0 row below |
...
- [x] Close every P0/P1 row above with red-then-green tests before #364 is described as shipped or merge-ready. **P1 and P2 rows closed 2026-08-19** (evidence in the table and the Phase 7–10 checklist entries); the three P0 ingestion rows are the remaining work.
...
- [ ] Re-run the exact `make test` / `make lint` gates once the three P0 ingestion rows land.
