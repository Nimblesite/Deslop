...
## The ranked build stall [PERF-FLUTTER-TODO-SUBSUME]

Measured on 3 September 2026 at commit bb17d9c on Windows (Ryzen 9 3900X, 24 threads, 32 GB), release build, the gate's own flags. The gate test exited 1 after 1,034 s of a 700 s ceiling without writing a report; a second run with the log kept was still inside ranked build when killed at 1,983 s. Every earlier stage completed by 301 s: discovery 1.3 s, parse and fingerprint 39.8 s, candidate pairs 88.9 s, trees 8.2 s, rescue 37.2 s, content gate 55.5 s, closure 1.7 s, noise split 65.3 s, handing 217,045 clusters to ranked build. Private bytes peaked at 8,384 MB (working set 7,004 MB) at 300 s and stayed flat; after that the process used one CPU-second per wall-second.

The profiler (Visual Studio collector, imported with samply, symbolised from the release PDB) on the packages/flutter/test subtree put 84.9% of main-thread samples under the ranked-build call, 42.7% in is_core_of and 21.5% in the subsumption scan loop. Cross-cluster subsumption compared every ranked cluster with every later one — 2.4 × 10¹⁰ pair evaluations per pass — and #494 added a linear core search per straddle candidate plus a fix-point rescan of the whole list.

Fixed by resolving each file set on its own ([PIPELINE-CLUSTER-SUBSUME-FILESET]): only views over exactly the same files can re-describe or straddle one another, so the verdicts are unchanged and the cost is a sum of small squares. The stage now emits completion and progress records.

Pinning the change exposed an accuracy defect in the same code ([PIPELINE-CLUSTER-SUBSUME], issue #498): a view that yielded through a drop-outer verdict was never released when its survivor died in a straddle, so a detected duplication left the report depending on the id order of equal-mass views. A complete release ledger loops forever under the survivor order, because three crossed views can each outrank the next. The scan was replaced by a per-file-set kernel ([PIPELINE-CLUSTER-SUBSUME-KERNEL]): a view is published when no published view re-describes and outranks it, so a view whose absorber leaves the set is judged again against what remains, and a cycle is decided by the coverage-mass-id tie-break ([PIPELINE-CLUSTER-SUBSUME-CYCLE]). Every subsumption test now holds its result to a spec-derived report contract.
