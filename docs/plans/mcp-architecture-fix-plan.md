...
- [x] **Coverage thresholds**: `coverage-thresholds.json` restores the Rust `ignore_filename_regex` source of truth for spawned-process runtime paths (live service support, LSP socket/server entrypoints, MCP IPC bridge, and Ollama-gated provider code). Current `make test`: `deslop-core` 96.3%, `deslop` 96.0%, `deslop-lsp` 96.2%, `deslop-mcp` 97.1% — all clear `threshold + 1% slack`.
...
- [x] **Rust validation clean**: `cargo fmt --all --check`, `cargo clippy --release --all-targets --workspace -- -D warnings`, `bash scripts/taxonomy-gate.sh`, and `make test` all pass. `make lint` passed earlier; the direct clippy/taxonomy rerun was used after `Makefile` became locked for the JetBrains package gate.

### 🟡 Follow-on (next session)

- [ ] **`make ci` clean**: run the full sequence once the concurrent JetBrains package-gate edits land and `Makefile` is unlocked.
