...
suite("cluster detail panel selection (#173)", () => {
  ...
  test("a surviving projection preserves the opened cluster's original canonical and figures", () => {
    ...
    const store = new ReportStore();
    store.setSnapshot(reportOf([original]), FIRST_OCCURRENCE_INDEX);
    ...
    const feed = clusterPanelFeed(reportOf([original]), visible, anchorForClusterId(reportOf([original]), original.id));
    assert.equal(feed.selectedId, original.id);
    assert.deepEqual(feed.report.clusters[FIRST_OCCURRENCE_INDEX], original);
