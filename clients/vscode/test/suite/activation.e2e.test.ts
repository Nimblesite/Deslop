...
suite("activation", () => {
  ...
  test("activity bar view container is registered", async () => {
    ...
    assert.ok(commands.includes("codededup.openReport"));
    assert.ok(commands.includes("codededup.openWorstCluster"));
    assert.ok(commands.includes("codededup.pickEmbeddingModel"));
    assert.ok(commands.includes("codededup.revealActiveBinary"));
  });
