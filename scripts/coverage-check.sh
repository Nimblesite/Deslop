...
if [[ ! -f "$LCOV" ]]; then
  echo "FAIL: $LCOV not found" >&2
  exit 1
fi
if [[ ! -f "$THRESHOLDS" ]]; then
  echo "FAIL: $THRESHOLDS not found" >&2
  exit 1
fi

DEFAULT=$(jq -r '.rust.default_threshold' "$THRESHOLDS")
if [[ "$DEFAULT" == "null" || -z "$DEFAULT" ]]; then
  echo "FAIL: $THRESHOLDS is missing .rust.default_threshold" >&2
  exit 1
fi

# Every workspace crate must have an entry; drift is a config bug.
...
for crate in "${CRATES[@]}"; do
  ...
  if [[ "$threshold" == "null" || -z "$threshold" ]]; then
    echo "FAIL: no threshold for crate $crate in $THRESHOLDS" >&2
    exit 1
  fi

  # Awk splits lcov.info by SF: blocks; sum LH+LF for files whose path
  # contains `crates/<crate>/src/`.
  read -r lh lf < <(awk -v crate="crates/$crate/src/" '
  ...
  if [[ "$lf" -eq 0 ]]; then
    echo "FAIL: crate $crate has no covered lines in $LCOV (all files filtered or crate has no tested source)" >&2
    exit 1
  fi
