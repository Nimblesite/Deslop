...
Deslop 从第一次提交起就将编码智能体设计为一等受众。MCP 与 LSP 外壳如今均已发布——两者都消费同一套 `deslop-core` 流水线、同一套 JSON schema，以及与 CLI 相同的磁盘缓存。每一个 MCP 工具响应都针对**实时**工作区状态计算——LSP 在内存中持有实时报告，并在每次变更时刷新（250 ms 防抖，2 s 上限），MCP 服务器则在下一次工具调用时通过 Unix 域套接字（`.deslop-cache/deslop.sock`）读取该实时状态。没有批处理步骤。没有陈旧缓存。
