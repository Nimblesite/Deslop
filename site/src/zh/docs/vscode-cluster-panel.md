...
`fused` 是 Deslop 的综合克隆评分。它将结构、词元和嵌入证据结合在一起，是用于判定某一对是否值得报告的评分。形状匹配会被下方的内容证据折扣，因此即使结构评分完美，`fused` 也可能远低于它。

## 内容证据

仅凭形状无法区分“重命名后的副本”和“恰好共用同一骨架的无关代码”。两个簇可以同样得到 `structural 1.00` 和 `jaccard 1.00`，但其中一个是真正的重复，另一个只是同级样板代码——相同的 `if/else` 骨架里包着完全不同的代码。内容证据是 Deslop 在匹配内部实际测量到的结果，也是把形状评分折扣成融合置信度的依据。

面板会在这些条形图下方给出两者的自然语言解读，因此你无需自己计算：它会给出形状评分、测得的一致度，以及二者得出的置信度。

## 一致度

`agreement` 表示这些位置在匹配内容上逐字节真正共享的比例。高形状评分下的低一致度意味着骨架对上了，但里面的代码没有。

## 重命名一致性

`rename` 表示是否有一次一致的标识符重命名可以解释这些位置之间的全部差异。这正是区分真正重命名副本与仅仅共用形状的无关代码的依据：`agreement 0.10` 且 `rename 1.00` 的簇是同一份代码换了名字，值得抽取。
