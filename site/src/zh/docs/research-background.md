...
当前由 `crates/deslop-core/src/pipeline/corpus.rs::default_parsers` 注册的受支持语言解析器有：C#、Rust、Python 和 Dart。TypeScript、JavaScript、Go 及其他语言尚未在当前核心流水线中注册。
...
| 主张 | 在代码中验证 | 有用的测试 |
| --- | --- | --- |
| C#、Rust、Python 和 Dart 目前已注册。 | `crates/deslop-core/src/pipeline/corpus.rs::default_parsers` | `cargo test -p deslop --test cli detects_type2_clone_in_csharp_fixture`、`cargo test -p deslop --test cli detects_type2_clone_in_rust_fixture`、`cargo test -p deslop --test cli detects_type2_clone_in_python_fixture` |
