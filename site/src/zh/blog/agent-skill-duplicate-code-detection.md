...
所以我们把自己的缺陷公开出来。[议题关系图](/issues/)展示所有未关闭的议题以及它们之间的关联；[规划页](/issues/planner/)展示工作跑道与排序队列，用透明的默认工作量估算，而不是承诺的日期；整份数据以机器可读的形式发布在 [`/assets/data/issues.json`](/assets/data/issues.json)——这意味着技能可以把实时的缺陷清单交给智能体，而不是一份在技能写完那一周就已经过期的快照。#364 就在里面。任何当前影响准确性的问题也在里面，与[准确性与透明度页面](/docs/accuracy-transparency/)上公布的实测数字并列。
