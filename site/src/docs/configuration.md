...
- Any file that opens with a generator's banner — a whole comment line above the first code, such as `// GENERATED CODE - DO NOT MODIFY BY HAND`, `// Code generated … DO NOT EDIT.`, `// <auto-generated>`, `// @generated` or `# Generated by the protocol buffer compiler.  DO NOT EDIT!`. A phrase inside a sentence, a string, or a comment further down never counts, so a hand-written file that merely mentions generators stays in the report. The [full banner list](https://github.com/Nimblesite/Deslop/blob/main/docs/specs/exclusion.md) is in the spec; a generator that is not on it is one `report_hide` pattern away.
