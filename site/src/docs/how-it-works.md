...
Identical Merkle hashes across files or within the same file form a Type-1 / Type-2 cluster immediately. This pass is O(n) and finds the most expensive duplication without any approximate matching.
...
Optional. When enabled, each subtree is run through a code-embedding model (local Ollama by default, configurable). Nearest-neighbour search via ANN produces Type-4 candidates — semantically equivalent but syntactically different code, such as an imperative loop versus a LINQ expression.
...
| --- | --- | --- |
| `structural` | 0..1 | Type-1, Type-2 (exact fingerprint match) |
