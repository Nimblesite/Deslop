...
3. The spec-compliant CI runs `make fmt CHECK=1 → make lint → make test → make build`. Confirm the actual file matches.
4. Note any environment variables, matrix strategies, or conditional steps that affect execution.

**Do NOT assume the steps.** Extract what the CI *actually does*. Flag any extras beyond the 7 standard Make targets.

## Step 3 — Run each CI step locally, in order

Work through failures in this priority order:

1. **Formatting** — `make fmt` to clear noise first
...
- Skip steps that are CI-infrastructure-only (checkout, setup-rust, cache, artifact uploads) — focus on the actual build/test/lint commands
